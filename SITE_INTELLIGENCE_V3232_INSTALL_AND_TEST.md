...
BUNDLE="$(find . -maxdepth 1 -type f \
  -name 'sustainable-catalyst-site-intelligence-v3.23.6.4-release-bundle*.zip' \
  -print0 | xargs -0 ls -t | head -1)"
...
Install the WordPress ZIP only after Terminal reports that v3.23.6.4 is live. Replace the current plugin, purge WordPress and CDN caches, and open the page in a new Incognito window.
