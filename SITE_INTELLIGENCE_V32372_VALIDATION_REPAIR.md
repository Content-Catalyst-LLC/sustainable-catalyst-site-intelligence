# Site Intelligence v3.23.8 Validation Repair

The original v3.23.8 installer stopped before promotion because its headless-Chrome gate assumed that pressing Home and End on a closed native `<select>` would change the selected value on every operating system.
...
The internal release remains v3.23.8 because the failed installer never reached GitHub, Render, or the WordPress handoff.
