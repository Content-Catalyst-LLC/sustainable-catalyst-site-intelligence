...
PYTHON="$ROOT/.venv/bin/python" bash "$ROOT/verify_site_intelligence_v3_23_3_macos.sh"
printf '\n==> Running deterministic validation pass 2\n'
PYTHON="$ROOT/.venv/bin/python" bash "$ROOT/verify_site_intelligence_v3_23_3_macos.sh"
printf '\n==> Promoting the backend through GitHub and Render\n'
PYTHON="$ROOT/.venv/bin/python" bash "$ROOT/promote_site_intelligence_v3_23_3_to_github_and_render_macos.sh"
...
SUCCESS: Site Intelligence v${RELEASE} is live with data freshness, coverage, and source truth.
...
Install the WordPress ZIP only now that the live gate has verified the exact release id, Git commit, app shell, map engine, interaction controls, data-truth contract, source-truth browser asset, production-truth directory, local geography, and runtime health.
DONE
