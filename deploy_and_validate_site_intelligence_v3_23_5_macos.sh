...
fail(){ printf '\nERROR: %s\n' "$1" >&2; exit 1; }
if [[ -z "$BUNDLE" ]]; then
  BUNDLE="$(find "$HOME/Downloads" -maxdepth 1 -type f -name 'sustainable-catalyst-site-intelligence-v3.23.7.1-release-bundle*.zip' -print0 2>/dev/null | xargs -0 ls -t 2>/dev/null | head -1 || true)"
fi
...
if [[ -f "$TARGET/SHA256SUMS.txt" ]]; then BUNDLE_ROOT="$TARGET"; else
  ...
  for candidate in "$TARGET"/*/SHA256SUMS.txt; do [[ -f "$candidate" ]] || continue; CHECKSUM_FILE="$candidate"; CHECKSUM_COUNT=$((CHECKSUM_COUNT+1)); done
...
PYTHON="$ROOT/.venv/bin/python" bash "$ROOT/verify_site_intelligence_v3_23_5_macos.sh"
printf '\n==> Running deterministic validation pass 2\n'
PYTHON="$ROOT/.venv/bin/python" bash "$ROOT/verify_site_intelligence_v3_23_5_macos.sh"
printf '\n==> Promoting the backend through GitHub and Render\n'
PYTHON="$ROOT/.venv/bin/python" bash "$ROOT/promote_site_intelligence_v3_23_5_to_github_and_render_macos.sh"
...
SUCCESS: Site Intelligence v${RELEASE} is live with browser reliability, mobile, and accessibility hardening.
...
Install the WordPress ZIP only now that the live gate has verified the exact release, Git commit, application shell, maps, analytical workflows, browser reliability contract, production truth, and runtime health.
DONE
