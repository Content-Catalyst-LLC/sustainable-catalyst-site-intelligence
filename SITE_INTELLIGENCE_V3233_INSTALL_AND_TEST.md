...
BUNDLE="$(find . -maxdepth 1 -type f -name 'sustainable-catalyst-site-intelligence-v3.23.6.1-release-bundle*.zip' -print0 | xargs -0 ls -t | head -1)"

chmod +x "$INSTALLER"
bash "$INSTALLER" "$BUNDLE"
```

Install the WordPress ZIP only after Terminal reports that v3.23.6.1 is live.
