...
PY
if command -v node >/dev/null 2>&1; then node "$ROOT/scripts/check_javascript_v42500.js" "$BACKEND/public_app" "$ROOT/wordpress-plugin/sustainable-catalyst-site-intelligence/assets"; fi
...
if [[ "${SC_SI_RUN_BROWSER:-0}" == "1" ]]; then PYTHON="$PYTHON" "$PYTHON" "$ROOT/scripts/browser_agriculture_v42500.py"; fi
echo 'SUCCESS: Site Intelligence v4.25.0 passed Agriculture, Crops & Food-System Conditions Intelligence validation.'
