...
ARCHIVE="${1:-/tmp/sustainable-catalyst-site-intelligence-backend-v4.40.0.3.zip}"
...
TMP="$(mktemp -d /tmp/sc-site-v44003.XXXXXX)"
trap 'rm -rf "$TMP"' EXIT
...
grep -q "RELEASE_NAME = \"Live Intelligence Ticker Pace & Readability Repair\"" "$version_file" || fail "release name mismatch"
...
grep -q "live_feed_count=len(LIVE_INTELLIGENCE_FEED_REGISTRY)" "$SRC_BACKEND/app/main.py" || fail "governed live-feed runtime binding missing"
for marker in enabled_connectors public_workspaces live_feeds featured_signal_count; do
  grep -q "$marker" "$SRC_BACKEND/app/homepage_summary_v4390.py" || fail "homepage capability marker missing: $marker"
done
...
# Reconcile only the immutable release-bound registry/policy allowlist.
...
from app.live_intelligence_reliability_v361 import FEED_REGISTRY
...
summary = build_homepage_summary(
    ...
    live_feed_count=len(FEED_REGISTRY),
)
...
assert summary['featured_signal_count'] == 1
paths={getattr(r,'path',None) for r in app.routes}
assert '/v1/public/site-intelligence/summary' in paths
assert '/public/live-intelligence/status' in paths
assert '/public/live-intelligence/homepage' in paths
print('PASS: v4.40.0.3 capability contract remains aligned while ticker pace release identity is active')
print(metrics)
PYVERIFY
...
python3 - "$health" <<'PY'
import json,sys
x=json.loads(sys.argv[1]); assert x.get('ok') is True; assert x.get('version')=='4.40.0.3', x
print('PASS: local health reports Site Intelligence 4.40.0.3')
PY
...
expected={'country_profiles':172,'enabled_connectors':14,'public_workspaces':35,'live_feeds':8}
assert m == expected, (m,expected)
assert isinstance(x.get('featured_signal_count'),int) and x['featured_signal_count'] >= 0
print('PASS: local homepage summary restores 172 / 14 / 35 / 8 capability contract')
print(json.dumps(m,indent=2))
print('featured_signal_count:', x['featured_signal_count'])
PY

echo "=== LOCAL TICKER FEED ==="
ticker="$(curl -fsS "http://127.0.0.1:${PORT}/public/live-intelligence/homepage?limit=8&channel=global")"
python3 - "$ticker" <<'PY'
import json,sys
x=json.loads(sys.argv[1]); assert x.get('ok') is True; assert x.get('version')=='4.40.0.3', x
signals=x.get('signals') or []
assert isinstance(signals,list) and len(signals) > 0, x
print('PASS: local governed homepage ticker feed returns signals:', len(signals))
PY

echo "=== LOCAL RELEASE GATE ==="
gate="$(curl -fsS "http://127.0.0.1:${PORT}/public/release-gate?plugin_version=4.40.0.3&expected_release_id=site-intelligence-v4.40.0.3")"
python3 - "$gate" <<'PY'
import json,sys
x=json.loads(sys.argv[1]); assert x.get('ok') is True; assert x.get('install_allowed') is True; assert x.get('version')=='4.40.0.3', x
print('PASS: local release gate allows WordPress 4.40.0.3')
PY
...
import json,sys
x=json.loads(sys.argv[1]); raise SystemExit(0 if x.get('version')=='4.40.0.3' else 1)
PY
    then break; fi
    sleep 3
  done
...
import json,sys
x=json.loads(sys.argv[1]); assert x.get('ok') is True; assert x.get('version')=='4.40.0.3', x
print('PASS: public backend health reports 4.40.0.3')
PY
...
import json,sys
x=json.loads(sys.argv[1]); assert x.get('version')=='4.40.0.3', x
m={str(v.get('id')):v.get('value') for v in x.get('metrics',[]) if isinstance(v,dict)}
expected={'country_profiles':172,'enabled_connectors':14,'public_workspaces':35,'live_feeds':8}
assert m == expected, (m,expected)
assert isinstance(x.get('featured_signal_count'),int) and x['featured_signal_count'] >= 0
print('PASS: public homepage summary restores 172 / 14 / 35 / 8 capability contract')
print(json.dumps(m,indent=2))
PY

  echo "=== PUBLIC TICKER FEED ==="
  public_ticker="$(curl -fsS --max-time 30 "$PUBLIC_BASE/public/live-intelligence/homepage?limit=8&channel=global")"
  python3 - "$public_ticker" <<'PY'
import json,sys
x=json.loads(sys.argv[1]); assert x.get('version')=='4.40.0.3', x
signals=x.get('signals') or []
assert isinstance(signals,list) and len(signals)>0, x
print('PASS: public governed homepage ticker feed returns signals:',len(signals))
PY
fi

echo "PASS: $PRODUCT v$VERSION backend deployed and verified."
echo "Backup: $BACKUP"
echo "NEXT: install the v$VERSION WordPress plugin, clear caches, then verify 172 / 14 / 35 / 8 plus the readable distance-paced ticker in the homepage widget."
