...
"$PYTHON" "$ROOT/scripts/validate_v41500_release.py"
...
PY
if command -v node >/dev/null 2>&1; then node "$ROOT/scripts/check_javascript_v41500.js" "$BACKEND/public_app" "$ROOT/wordpress-plugin/sustainable-catalyst-site-intelligence/assets"; fi
...
if [[ "${SC_SI_RUN_BROWSER:-0}" == "1" ]]; then PYTHON="$PYTHON" "$PYTHON" "$ROOT/scripts/browser_ocean_governance_v41500.py"; fi
echo 'SUCCESS: Site Intelligence v4.15.0 passed Ocean Governance, Jurisdiction & Maritime Boundaries validation.'
