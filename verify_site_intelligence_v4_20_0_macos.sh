...
PY
if command -v node >/dev/null 2>&1; then node "$ROOT/scripts/check_javascript_v42000.js" "$BACKEND/public_app" "$ROOT/wordpress-plugin/sustainable-catalyst-site-intelligence/assets"; fi
...
if [[ "${SC_SI_RUN_BROWSER:-0}" == "1" ]]; then PYTHON="$PYTHON" "$PYTHON" "$ROOT/scripts/browser_geosphere_v42000.py"; fi
echo 'SUCCESS: Site Intelligence v4.20.0 passed Global Geosphere, Earthquake & Volcano Intelligence validation.'
