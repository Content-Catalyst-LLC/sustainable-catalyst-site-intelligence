...
if [[ -x "$SOURCE_ROOT/.venv/bin/python" ]]; then PYTHON="$SOURCE_ROOT/.venv/bin/python" bash "$SOURCE_ROOT/verify_site_intelligence_v3_23_6_1_macos.sh"; else bash "$SOURCE_ROOT/verify_site_intelligence_v3_23_6_1_macos.sh"; fi
...
if [[ -x "$SOURCE_ROOT/.venv/bin/python" ]]; then PYTHON="$SOURCE_ROOT/.venv/bin/python" bash "$CLONE_ROOT/verify_site_intelligence_v3_23_6_1_macos.sh"; else bash "$CLONE_ROOT/verify_site_intelligence_v3_23_6_1_macos.sh"; fi
cd "$CLONE_ROOT"
...
if ! git diff --cached --quiet; then
  ...
  git commit -m "Site Intelligence v${RELEASE} — single-owner bootstrap and loading recovery"
fi
...
' "$OBSERVED_VERSION" "${OBSERVED_COMMIT:0:12}"
        printf '
SUCCESS: Site Intelligence v3.23.6.3 is live with single-owner bootstrap and loading recovery.
