...
PYTHON="$ROOT/.venv/bin/python" bash "$ROOT/verify_site_intelligence_v3_29_0_macos.sh"
printf '\n==> Running deterministic validation pass 2\n'
PYTHON="$ROOT/.venv/bin/python" bash "$ROOT/verify_site_intelligence_v3_29_0_macos.sh"
printf '\n==> Promoting the backend through GitHub and Render\n'
PYTHON="$ROOT/.venv/bin/python" bash "$ROOT/promote_site_intelligence_v3_29_0_to_github_and_render_macos.sh"
...
SUCCESS: Site Intelligence v${RELEASE} is live with the Briefing, Story Map, and Publication Studio.
...
Install the WordPress ZIP only now that the live gate has verified the exact release, Git commit, application shell, maps, analytical workflows, briefing/story-map/publication studio, monitoring/digest/early-warning operations, research evidence/knowledge integration, comparative/scenario/model assurance, browser reliability, performance/offline contract, single-owner bootstrap contract, mutation-observer recovery contract, mandatory complete-shell browser gate, service-worker strategy, production truth, and runtime health.
DONE
