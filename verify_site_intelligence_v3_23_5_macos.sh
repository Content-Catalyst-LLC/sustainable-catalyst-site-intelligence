...
printf '\n==> Verifying v3.23.5 browser reliability contracts\n'
...
root=Path(sys.argv[1]);m=json.loads((root/'MANIFEST.json').read_text());assert m['release']=='3.23.5';assert m['file_count']==len(m['files'])
for e in m['files']:
...
if [[ "${SC_SI_SKIP_BROWSER_SMOKE:-0}" != "1" ]]; then
  printf '\n==> Running v3.23.5 Chromium reliability presentation check when available\n'
