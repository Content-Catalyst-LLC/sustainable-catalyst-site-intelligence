...
if [[ -x "$SOURCE_ROOT/.venv/bin/python" ]]; then PYTHON="$SOURCE_ROOT/.venv/bin/python" bash "$SOURCE_ROOT/verify_site_intelligence_v3_23_6_3_macos.sh"; else bash "$SOURCE_ROOT/verify_site_intelligence_v3_23_6_3_macos.sh"; fi
...
if [[ -x "$SOURCE_ROOT/.venv/bin/python" ]]; then PYTHON="$SOURCE_ROOT/.venv/bin/python" bash "$CLONE_ROOT/verify_site_intelligence_v3_23_6_3_macos.sh"; else bash "$CLONE_ROOT/verify_site_intelligence_v3_23_6_3_macos.sh"; fi
cd "$CLONE_ROOT"
...
if ! git diff --cached --quiet; then
  ...
  git commit -m "Site Intelligence v${RELEASE} — fixed application viewport and WordPress embed isolation"
fi
...
for ((attempt=1; attempt<=ATTEMPTS; attempt++)); do
  ...
  if [[ -n "$payload" ]]; then
    ...
    if [[ "$OBSERVED_VERSION" == "$RELEASE" && "$OBSERVED_RELEASE_ID" == "$RELEASE_ID" && "$OBSERVED_GATE" == "ready" && ( "$OBSERVED_COMMIT" == "$COMMIT" || "$COMMIT" == "$OBSERVED_COMMIT"* || "$OBSERVED_COMMIT" == "$COMMIT"* ) ]]; then
      ...
      app_ready=false; engine_ready=false; world_ready=false; workspace_ready=false; interaction_ready=false; data_truth_ready=false; analytical_ready=false; browser_reliability_ready=false; performance_offline_ready=false; bootstrap_recovery_ready=false; mutation_recovery_ready=false; embed_isolation_ready=false; truth_ready=false; health_ready=false
      [[ "$app_html" == *'data-scsi-release="3.23.6.3"'* && "$app_html" == *'vector-cartography-v3230.js'* && "$app_html" == *'cartographic-workspace-v3230.js'* && "$app_html" == *'cartographic-interaction-v3232.js'* ]] && app_ready=true
      ...
      interaction_ready="$(python3 -c 'import json,sys; d=json.load(sys.stdin); print("true" if d.get("ok") and d.get("version")=="3.23.6.3" and all(d.get("layer_controls",{}).values()) else "false")' <<<"$interaction_contract" 2>/dev/null || printf false)"
      ...
      data_truth_ready="$(python3 -c 'import json,sys; d=json.load(sys.stdin); p=d.get("classification_policy",{}); print("true" if d.get("ok") and d.get("version")=="3.23.6.3" and d.get("source_count")==8 and p.get("cached_is_live") is False and p.get("demonstration_is_live") is False else "false")' <<<"$data_truth_contract" 2>/dev/null || printf false)"
      [[ "$data_truth_js" == *'SCSIDataTruthV3233'* && "$data_truth_js" == *'stale_marker_required'* && "$data_truth_js" == *'dataTruthPanel'* ]] || data_truth_ready=false
      analytical_ready="$(python3 -c 'import json,sys;d=json.load(sys.stdin);print("true" if d.get("ok") and d.get("version")=="3.23.6.3" and d.get("workflow_count")==5 and d.get("summary",{}).get("unavailable")==0 else "false")' <<<"$analytical_contract" 2>/dev/null || printf false)"
      ...
      browser_reliability_ready="$(python3 -c 'import json,sys;d=json.load(sys.stdin);print("true" if d.get("ok") and d.get("version")=="3.23.6.3" and d.get("contract")=="browser-reliability-mobile-accessibility" and d.get("accessibility",{}).get("map_text_summaries") else "false")' <<<"$browser_reliability_contract" 2>/dev/null || printf false)"
      ...
      performance_offline_ready="$(python3 -c 'import json,sys;d=json.load(sys.stdin);print("true" if d.get("ok") and d.get("version")=="3.23.6.3" and d.get("contract")=="performance-and-offline-recovery" and d.get("loading",{}).get("route_request_cancellation") else "false")' <<<"$performance_offline_contract" 2>/dev/null || printf false)"
      ...
      bootstrap_recovery_ready="$(python3 -c 'import json,sys;d=json.load(sys.stdin);print("true" if d.get("ok") and d.get("version")=="3.23.6.3" and d.get("contract")=="single-owner-bootstrap-and-loading-recovery" and d.get("service_worker",{}).get("registration_owner_count")==1 else "false")' <<<"$bootstrap_recovery_contract" 2>/dev/null || printf false)"
      ...
      embed_isolation_ready="$(python3 -c 'import json,sys;d=json.load(sys.stdin);a=d.get("application_embed",{});m=d.get("message_policy",{});print("true" if d.get("ok") and d.get("version")=="3.23.6.3" and a.get("document_auto_resize") is False and a.get("internal_scrolling") and m.get("child_height_messages_enabled") is False else "false")' <<<"$embed_isolation_contract" 2>/dev/null || printf false)"
      truth_ready="$(python3 -c 'import json,sys; d=json.load(sys.stdin); print("true" if d.get("ok") and d.get("version")=="3.23.6.3" and d.get("route_count")==35 and d.get("summary",{}).get("unavailable")==0 else "false")' <<<"$truth_contract" 2>/dev/null || printf false)"
...
' "$OBSERVED_VERSION" "${OBSERVED_COMMIT:0:12}"
        printf '
SUCCESS: Site Intelligence v3.23.6.3 is live with fixed application viewport and WordPress embed isolation.
