...
for ((attempt=1; attempt<=ATTEMPTS; attempt++)); do
  ...
  if [[ -n "$payload" ]]; then
    ...
    if [[ "$OBSERVED_VERSION" == "$RELEASE" && "$OBSERVED_RELEASE_ID" == "$RELEASE_ID" && "$OBSERVED_GATE" == "ready" && ( "$OBSERVED_COMMIT" == "$COMMIT" || "$COMMIT" == "$OBSERVED_COMMIT"* || "$OBSERVED_COMMIT" == "$COMMIT"* ) ]]; then
      ...
      browser_reliability_ready="$(python3 -c 'import json,sys;d=json.load(sys.stdin);print("true" if d.get("ok") and d.get("version")=="4.10.0" and d.get("contract")=="browser-reliability-mobile-accessibility" and d.get("accessibility",{}).get("map_text_summaries") else "false")' <<<"$browser_reliability_contract" 2>/dev/null || printf false)"
...
' "$OBSERVED_VERSION" "${OBSERVED_COMMIT:0:12}"
        printf '
SUCCESS: Site Intelligence v4.10.0 is live with country selector hydration and global selection repair.
