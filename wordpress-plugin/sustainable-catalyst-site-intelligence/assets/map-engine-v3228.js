(function (window, document) {
  ...
  function registerMap(map, mode) {
    ...
    container.classList.add("scsi-map-managed", "scsi-self-hosted-map");
    container.dataset.scsiMapMode = mode || "self-hosted-local-vector";
  ...
  class SelfHostedMap {
    constructor(target, options = {}) {
      ...
      this._canvas.innerHTML = '<div class="scsi-map-tile-root" aria-hidden="true"></div><svg class="scsi-map-engine-svg" role="img" aria-label="Map with local country boundaries and evidence overlays"><g class="scsi-local-basemap"></g><g class="scsi-map-grid"></g><g class="scsi-map-overlay-root"></g></svg><div class="scsi-map-html-markers"></div><div class="scsi-map-engine-label">Self-hosted map · local world boundaries</div><div class="scsi-map-attribution">Natural Earth · OpenStreetMap when available</div><div class="scsi-map-controls" role="group" aria-label="Map controls"><button type="button" data-map-zoom-in aria-label="Zoom in">+</button><button type="button" data-map-zoom-out aria-label="Zoom out">−</button><button type="button" data-map-reset aria-label="Reset world view">⌂</button></div><div class="scsi-map-popup" hidden></div>';
      ...
      registerMap(this, "self-hosted-local-vector");
      ...
      this._redraw();
      dispatch("scsi:map-first-party-ready", { containerId: containerId(this._container), mode: "self-hosted-local-vector" });
    }
    _dimensions() {
    ...
    _drawBoundaries() {
      ...
      this._boundaries.features.forEach(feature => {
        ...
        this._geometryPaths(feature.geometry).forEach(coords => {
          ...
          path.setAttribute("data-country", feature.properties?.name || "");
          const title = document.createElementNS(ns, "title"); title.textContent = feature.properties?.name || "Country boundary"; path.appendChild(title);
    ...
    _redraw() {
      ...
      this._drawBoundaries(); this._drawGrid(); this._layers.forEach(layer => layer?._redraw?.());
  ...
  class SelfHostedTileLayer {
    ...
    _attach(map) { this._map = map; this._root = document.createElement("div"); this._root.className = "scsi-map-tile-layer"; this._root.style.opacity = this.options.opacity ?? 1; map._tileRoot.appendChild(this._root); this._redraw(); }
    ...
    _settled() {
      ...
      if (this._loaded > 0) {
        container.classList.remove("scsi-map-imagery-limited"); container.dataset.scsiMapMode = "self-hosted-tiles"; container.dataset.scsiImageryMode = "normal"; container.dataset.scsiMapStatus = "ready";
        this._map._label.textContent = "Self-hosted map · live basemap tiles + local boundaries"; this._emit("load"); dispatch("scsi:map-recovered", { containerId: containerId(container), mode: "self-hosted-tiles", source: this.url }); return;
      }
      ...
      container.dataset.scsiMapMode = this._map._boundaries ? "self-hosted-local-vector" : "self-hosted-grid";
      this._map._label.textContent = this._map._boundaries ? "Self-hosted map · local world boundaries" : "Self-hosted map · geographic grid";
  ...
  const L = {
    version: `scsi-self-hosted-${VERSION}`,
  ...
  window.SCSIMapReliability = {
    ...
    snapshot: function () {
      ...
      const surfaces = containers.map(container => {
        ...
        return { id: containerId(container), mode: container.dataset.scsiMapMode || "self-hosted-local-vector", status: degraded ? "degraded" : "ready", degraded, imageryMode: container.dataset.scsiImageryMode || "normal", visible, localBasemap: localReady ? "ready" : (container.dataset.scsiLocalBasemap || "loading"), recoveryScheduled: false };
      });
      return { version: VERSION, libraryMode: "self-hosted-map-engine", mapCount: containers.length, degradedCount: surfaces.filter(item => item.degraded).length, imageryLimitedCount: surfaces.filter(item => item.imageryMode === "limited").length, modes: surfaces.reduce((counts, item) => { counts[item.mode] = (counts[item.mode] || 0) + 1; return counts; }, {}), surfaces };
    },
  ...
  dispatch("scsi:map-library-ready", { mode: "self-hosted-map-engine", leafletVersion: L.version, firstParty: true, localBoundaries: true });
})(window, document);
