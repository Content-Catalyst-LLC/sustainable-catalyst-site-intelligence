(function () {
  ...
  function readRecoveredJson(url) {
    try {
      ...
      window.dispatchEvent(new CustomEvent('scsi:service-fallback', {detail: {version: cfg.version || '4.35.10', group: 'wordpress-proxy', path: url, reason: 'last-known-good', staleAgeMs: Date.now() - item.savedAt}}));
      return item.data;
    } catch (e) { return null; }
  }
    ...
    function run() {
      ...
      return fetchJsonAttempt(url).catch(function (error) {
        ...
        if (retryable && attempt < 3) {
          window.dispatchEvent(new CustomEvent('scsi:service-retry', {detail: {version: cfg.version || '4.35.10', group: 'wordpress-proxy', path: url, attempt: attempt + 1}}));
