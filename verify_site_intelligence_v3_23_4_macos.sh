...
printf '
==> Verifying v3.23.4 analytical workspace contracts
'
...
root=Path(sys.argv[1]);m=json.loads((root/'MANIFEST.json').read_text());assert m['release']=='3.23.4';assert m['file_count']==len(m['files'])
for e in m['files']:
