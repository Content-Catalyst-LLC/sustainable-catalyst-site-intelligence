...
printf '\n==> Running complete inherited and v4.7.0 test suite with bounded process-isolated teardown\n'
...
printf '
==> Verifying v4.7.0 Global Data Truth Control Plane
'
...
grep -q 'record-provenance-v3238.js?v=4.7.0' "$BACKEND/public_app/index.html"
...
grep -q 'data-truth-control-plane-v3240.js?v=4.7.0' "$BACKEND/public_app/index.html"
...
printf '\n==> Running mandatory v4.7.0 record provenance and indicator-truth gate\n'
reset_runtime
"$PYTHON" "$ROOT/scripts/run_browser_gate_v3240.py" browser_record_provenance_v3238.py
printf '
==> Running mandatory v4.7.0 Global Data Truth Control Plane gate
'
reset_runtime
...
printf '
SUCCESS: Site Intelligence v4.7.0 passed Global Data Truth Control Plane validation.
Repository: %s
' "$ROOT"
