...
if [[ -x "$SOURCE_ROOT/.venv/bin/python" ]]; then PYTHON="$SOURCE_ROOT/.venv/bin/python" bash "$SOURCE_ROOT/verify_site_intelligence_v3_23_2_macos.sh"; else bash "$SOURCE_ROOT/verify_site_intelligence_v3_23_2_macos.sh"; fi
...
if [[ -x "$SOURCE_ROOT/.venv/bin/python" ]]; then PYTHON="$SOURCE_ROOT/.venv/bin/python" bash "$CLONE_ROOT/verify_site_intelligence_v3_23_2_macos.sh"; else bash "$CLONE_ROOT/verify_site_intelligence_v3_23_2_macos.sh"; fi
cd "$CLONE_ROOT"
...
if ! git diff --cached --quiet; then
  ...
  git commit -m "Site Intelligence v${RELEASE} — cartographic interaction and layer control"
fi
...
for ((attempt=1; attempt<=ATTEMPTS; attempt++)); do
  ...
  if [[ -n "$payload" ]]; then
    ...
    if [[ "$OBSERVED_VERSION" == "$RELEASE" && "$OBSERVED_RELEASE_ID" == "$RELEASE_ID" && "$OBSERVED_GATE" == "ready" && ( "$OBSERVED_COMMIT" == "$COMMIT" || "$COMMIT" == "$OBSERVED_COMMIT"* || "$OBSERVED_COMMIT" == "$COMMIT"* ) ]]; then
      ...
      app_ready=false; engine_ready=false; world_ready=false; workspace_ready=false; interaction_ready=false; truth_ready=false; health_ready=false
      ...
      if [[ "$app_ready" == true && "$engine_ready" == true && "$world_ready" == true && "$workspace_ready" == true && "$interaction_ready" == true && "$truth_ready" == true && "$health_ready" == true ]]; then
        write_receipt "verified" "GitHub, Render, the live app shell, self-hosted map engine, cartographic workspace, interaction controls, production-truth directory, runtime health, and WordPress installation gate are synchronized."
        printf 'Render release gate and live map runtime are ready for v%s at commit %s.
' "$OBSERVED_VERSION" "${OBSERVED_COMMIT:0:12}"
        printf '
SUCCESS: Site Intelligence v3.23.6.3 is live with cartographic interaction and layer control.
