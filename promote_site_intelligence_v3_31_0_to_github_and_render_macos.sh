...
for ((attempt=1; attempt<=ATTEMPTS; attempt++)); do
  ...
  if [[ -n "$payload" ]]; then
    ...
    if [[ "$OBSERVED_VERSION" == "$RELEASE" && "$OBSERVED_RELEASE_ID" == "$RELEASE_ID" && "$OBSERVED_GATE" == "ready" && ( "$OBSERVED_COMMIT" == "$COMMIT" || "$COMMIT" == "$OBSERVED_COMMIT"* || "$OBSERVED_COMMIT" == "$COMMIT"* ) ]]; then
      ...
      [[ "$app_html" == *'data-scsi-release="3.31.0"'* && "$app_html" == *'vector-cartography-v3230.js'* && "$app_html" == *'cartographic-workspace-v3230.js'* && "$app_html" == *'cartographic-interaction-v3232.js'* && "$app_html" == *'data-truth-control-plane-v3240.js?v=3.31.0'* && "$app_html" == *'cross-view-state-v3250.js?v=3.31.0'* && "$app_html" == *'assurance-v3260.js?v=3.31.0'* && "$app_html" == *'research-integration-v3270.js?v=3.31.0'* ]] && app_ready=true
      selector_catalog_ready="$(python3 -c 'import json,sys; d=json.load(sys.stdin); print("true" if d.get("ok") and d.get("version")=="3.31.0" and d.get("country_count",0)>=170 else "false")' <<<"$country_catalog_contract" 2>/dev/null || printf false)"
      ...
      interaction_ready="$(python3 -c 'import json,sys; d=json.load(sys.stdin); print("true" if d.get("ok") and d.get("version")=="3.31.0" and all(d.get("layer_controls",{}).values()) else "false")' <<<"$interaction_contract" 2>/dev/null || printf false)"
      ...
      data_truth_ready="$(python3 -c 'import json,sys; d=json.load(sys.stdin); p=d.get("classification_policy",{}); print("true" if d.get("ok") and d.get("version")=="3.31.0" and d.get("source_count")==8 and p.get("cached_is_live") is False and p.get("demonstration_is_live") is False else "false")' <<<"$data_truth_contract" 2>/dev/null || printf false)"
      country_truth_ready="$(python3 -c 'import json,sys; d=json.load(sys.stdin); print("true" if d.get("ok") and d.get("version")=="3.31.0" and d.get("country",{}).get("code")=="BRA" and d.get("source_count")==8 else "false")' <<<"$country_truth_contract" 2>/dev/null || printf false)"
      country_matrix_ready="$(python3 -c 'import json,sys; d=json.load(sys.stdin); print("true" if d.get("ok") and d.get("version")=="3.31.0" and d.get("country_count")==6 and d.get("source_count")==8 else "false")' <<<"$country_matrix_contract" 2>/dev/null || printf false)"
      ...
      record_truth_indicator_ready="$(python3 -c 'import json,sys;d=json.load(sys.stdin);f=d.get("fingerprint",{}).get("value","");print("true" if d.get("ok") and d.get("version")=="3.31.0" and d.get("record_id")=="indicator:KEN:SP.POP.TOTL" and d.get("truth_state")=="historical_snapshot" and len(f)==64 else "false")' <<<"$record_truth_indicator" 2>/dev/null || printf false)"
      record_truth_manifest_ready="$(python3 -c 'import json,sys;d=json.load(sys.stdin);print("true" if d.get("ok") and d.get("version")=="3.31.0" and d.get("entry_count",0)>=13 and len(d.get("manifest_fingerprint", ""))==64 else "false")' <<<"$record_truth_manifest" 2>/dev/null || printf false)"
      ...
      control_plane_overview_ready="$(python3 -c 'import json,sys;d=json.load(sys.stdin);s=d.get("summary",{});print("true" if d.get("ok") and d.get("version")=="3.31.0" and d.get("source_count")==8 and len(d.get("control_plane_fingerprint", ""))==64 and sum(s.get(k,0) for k in ("operational","degraded","review","unavailable","unknown"))==8 else "false")' <<<"$control_plane_contract" 2>/dev/null || printf false)"
      control_plane_schema_ready="$(python3 -c 'import json,sys;d=json.load(sys.stdin);print("true" if d.get("ok") and d.get("version")=="3.31.0" and d.get("source_count")==8 else "false")' <<<"$control_plane_schema" 2>/dev/null || printf false)"
      control_plane_outages_ready="$(python3 -c 'import json,sys;d=json.load(sys.stdin);print("true" if d.get("ok") and d.get("version")=="3.31.0" and d.get("incident_count")==len(d.get("incidents",[])) else "false")' <<<"$control_plane_outages" 2>/dev/null || printf false)"
      control_plane_coverage_ready="$(python3 -c 'import json,sys;d=json.load(sys.stdin);print("true" if d.get("ok") and d.get("version")=="3.31.0" and d.get("country_count")==3 and d.get("source_count")==8 and sum(d.get("state_counts",{}).values())==24 else "false")' <<<"$control_plane_coverage" 2>/dev/null || printf false)"
      control_plane_workspaces_ready="$(python3 -c 'import json,sys;d=json.load(sys.stdin);print("true" if d.get("ok") and d.get("version")=="3.31.0" and d.get("country",{}).get("code")=="BRA" and d.get("workspace_count")==12 else "false")' <<<"$control_plane_workspaces" 2>/dev/null || printf false)"
      ...
      unified_state_contract_ready="$(python3 -c 'import json,sys;d=json.load(sys.stdin);print("true" if d.get("ok") and d.get("version")=="3.31.0" and d.get("contract")=="unified-analytical-workspace-state" and d.get("route_count")==6 and d.get("country_catalog_count",0)>=170 else "false")' <<<"$unified_state_contract" 2>/dev/null || printf false)"
      unified_handoff_ready="$(python3 -c 'import json,sys;d=json.load(sys.stdin);s=d.get("state",{});print("true" if d.get("ok") and d.get("version")=="3.31.0" and d.get("contract")=="cross-view-analytical-handoff" and d.get("target")=="compare" and s.get("country")=="BRA" and s.get("compare")=="IND" and "country=BRA" in d.get("path","") else "false")' <<<"$unified_handoff_contract" 2>/dev/null || printf false)"
      ...
      assurance_contract_ready="$(python3 -c 'import json,sys;d=json.load(sys.stdin);print("true" if d.get("ok") and d.get("version")=="3.31.0" and d.get("contract")=="comparative-scenario-model-assurance" and len(d.get("comparison_dimensions",[]))>=6 else "false")' <<<"$assurance_contract" 2>/dev/null || printf false)"
      assurance_cards_ready="$(python3 -c 'import json,sys;d=json.load(sys.stdin);print("true" if d.get("ok") and d.get("version")=="3.31.0" and d.get("method_card_count",0)>=2 else "false")' <<<"$assurance_cards" 2>/dev/null || printf false)"
      assurance_scenario_ready="$(python3 -c 'import json,sys;d=json.load(sys.stdin);u=d.get("uncertainty_envelope",{});print("true" if d.get("ok") and d.get("version")=="3.31.0" and d.get("contract")=="scenario-assurance-review" and d.get("base_outcome")==110 and u.get("probabilistic") is False else "false")' <<<"$assurance_scenario" 2>/dev/null || printf false)"
      ...
      research_integration_contract_ready="$(python3 -c 'import json,sys;d=json.load(sys.stdin);print("true" if d.get("ok") and d.get("version")=="3.31.0" and d.get("contract")=="research-evidence-and-knowledge-integration" and len(d.get("targets",{}))==4 and d.get("human_confirmation_required") is True and d.get("automatic_delivery") is False else "false")' <<<"$research_integration_contract" 2>/dev/null || printf false)"
      research_workbench_ready="$(python3 -c 'import json,sys;d=json.load(sys.stdin);p=d.get("packet",{});print("true" if d.get("ok") and d.get("version")=="3.31.0" and p.get("target")=="workbench" and p.get("preview_only") is True and p.get("delivery_attempted") is False and p.get("human_confirmation_required") is True else "false")' <<<"$research_workbench_preview" 2>/dev/null || printf false)"
      ...
      publication_studio_contract_ready="$(python3 -c 'import json,sys;d=json.load(sys.stdin);print("true" if d.get("ok") and d.get("version")=="3.31.0" and d.get("contract")=="briefing-story-map-publication-studio" and d.get("human_editorial_review_required") is True and d.get("automatic_publication") is False and d.get("public_write_performed") is False else "false")' <<<"$publication_studio_contract" 2>/dev/null || printf false)"
      [[ "$publication_studio_js" == *'SCSIBriefingPublicationV3290'* && "$publication_studio_contract_ready" == true ]] && publication_studio_ready=true
      institutional_governance_contract_ready="$(python3 -c 'import json,sys;d=json.load(sys.stdin);print("true" if d.get("ok") and d.get("version")=="3.31.0" and d.get("contract")=="institutional-workspaces-review-governance" and d.get("public_accounts_required") is False and d.get("paid_multi_tenant_infrastructure_required") is False and d.get("automatic_evidence_approval") is False and d.get("automatic_publication") is False and d.get("public_write_performed") is False else "false")' <<<"$institutional_governance_contract" 2>/dev/null || printf false)"
      ...
      analytical_ready="$(python3 -c 'import json,sys;d=json.load(sys.stdin);print("true" if d.get("ok") and d.get("version")=="3.31.0" and d.get("workflow_count")==5 and d.get("summary",{}).get("unavailable")==0 else "false")' <<<"$analytical_contract" 2>/dev/null || printf false)"
      ...
      browser_reliability_ready="$(python3 -c 'import json,sys;d=json.load(sys.stdin);a=d.get("accessibility",{});r=d.get("reliability",{});print("true" if d.get("ok") and d.get("version")=="3.31.0" and d.get("contract")=="browser-reliability-mobile-accessibility" and a.get("map_text_summaries") and a.get("form_control_focus_protection") and a.get("native_select_scroll_preserved") and a.get("route_focus_only_on_route_change") and r.get("repeated_workspace_state_does_not_refocus") else "false")' <<<"$browser_reliability_contract" 2>/dev/null || printf false)"
      ...
      performance_offline_ready="$(python3 -c 'import json,sys;d=json.load(sys.stdin);print("true" if d.get("ok") and d.get("version")=="3.31.0" and d.get("contract")=="performance-and-offline-recovery" and d.get("loading",{}).get("route_request_cancellation") else "false")' <<<"$performance_offline_contract" 2>/dev/null || printf false)"
      ...
      startup_stability_ready="$(python3 -c 'import json,sys;d=json.load(sys.stdin);s=d.get("startup",{});w=d.get("service_worker",{});r=d.get("route_stability",{});print("true" if d.get("ok") and d.get("version")=="3.31.0" and s.get("network_data_blocks_shell") is False and s.get("initial_data_strategy")=="background-all-settled" and w.get("automatic_controllerchange_reload") is False and w.get("install_calls_skip_waiting") is False and r.get("concurrent_route_transitions") is False else "false")' <<<"$startup_stability_contract" 2>/dev/null || printf false)"
      ...
      bootstrap_recovery_ready="$(python3 -c 'import json,sys;d=json.load(sys.stdin);print("true" if d.get("ok") and d.get("version")=="3.31.0" and d.get("contract")=="single-owner-bootstrap-and-loading-recovery" and d.get("service_worker",{}).get("registration_owner_count")==1 else "false")' <<<"$bootstrap_recovery_contract" 2>/dev/null || printf false)"
      ...
      embed_isolation_ready="$(python3 -c 'import json,sys;d=json.load(sys.stdin);a=d.get("application_embed",{});m=d.get("message_policy",{});print("true" if d.get("ok") and d.get("version")=="3.31.0" and a.get("document_auto_resize") is False and a.get("internal_scrolling") and m.get("child_height_messages_enabled") is False else "false")' <<<"$embed_isolation_contract" 2>/dev/null || printf false)"
      truth_ready="$(python3 -c 'import json,sys; d=json.load(sys.stdin); print("true" if d.get("ok") and d.get("version")=="3.31.0" and d.get("route_count")==35 and d.get("summary",{}).get("unavailable")==0 else "false")' <<<"$truth_contract" 2>/dev/null || printf false)"
      ...
      health_ready="$(python3 -c 'import json,sys; d=json.load(sys.stdin); print("true" if d.get("ok") and d.get("status")=="healthy" and d.get("version")=="3.31.0" else "false")' <<<"$runtime_health" 2>/dev/null || printf false)"
...
' "$OBSERVED_VERSION" "${OBSERVED_COMMIT:0:12}"
        printf '
SUCCESS: Site Intelligence v3.31.0 is live with Security, Observability, Performance, and Scale Assurance.
