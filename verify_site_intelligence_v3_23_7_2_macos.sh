...
grep -q 'const RELEASE="4.8.0"' "$BACKEND/public_app/service-worker.js"
grep -q '/public/data-truth/coverage-matrix' "$BACKEND/app/main.py"
grep -q 'data-truth-v32371.js?v=4.8.0' "$BACKEND/public_app/index.html"
...
root=Path(sys.argv[1]);m=json.loads((root/'MANIFEST.json').read_text());assert m['release']=='4.8.0';assert m['file_count']==len(m['files'])
for e in m['files']:
...
printf '\n==> Running mandatory v4.8.0 production-soak route and service-worker gate\n'
...
printf '
SUCCESS: Site Intelligence v4.8.0 passed country dropdown interaction and focus-safety validation.
Repository: %s
' "$ROOT"
