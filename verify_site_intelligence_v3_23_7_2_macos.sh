...
printf '
==> Running mandatory v4.4.0 country dropdown interaction and focus-safety gate
'
"$PYTHON" "$ROOT/scripts/run_browser_gate_v32372.py" browser_country_selector_interaction_v32372.py

printf '
==> Running mandatory v4.4.0 global country selector hydration gate
'
"$PYTHON" "$ROOT/scripts/run_browser_gate_v32372.py" browser_country_selector_hydration_v32372.py

printf '\n==> Running mandatory v4.4.0 global country truth browser gate\n'
"$PYTHON" "$ROOT/scripts/run_browser_gate_v32372.py" browser_global_country_data_truth_v32372.py

rm -rf "$BACKEND/backend"
printf '
==> Running complete inherited and v4.4.0 test suite with process-isolated teardown
'
PYTHONPATH="$BACKEND" PYTEST_DISABLE_PLUGIN_AUTOLOAD=1 "$PYTHON" "$ROOT/scripts/run_v32372_test_suite.py"
printf '
==> Verifying v4.4.0 country dropdown interaction and focus safety
'
"$PYTHON" "$ROOT/scripts/validate_v32372_release.py"
grep -q 'APP_VERSION = "4.4.0"' "$BACKEND/app/version.py"
grep -q 'Version: 4.4.0' "$ROOT/wordpress-plugin/sustainable-catalyst-site-intelligence/sustainable-catalyst-site-intelligence.php"
grep -q 'const RELEASE="4.4.0"' "$BACKEND/public_app/service-worker.js"
grep -q '/public/data-truth/coverage-matrix' "$BACKEND/app/main.py"
grep -q 'data-truth-v32371.js?v=4.4.0' "$BACKEND/public_app/index.html"
...
root=Path(sys.argv[1]);m=json.loads((root/'MANIFEST.json').read_text());assert m['release']=='4.4.0';assert m['file_count']==len(m['files'])
for e in m['files']:
...
printf '\n==> Running mandatory v4.4.0 production-soak route and service-worker gate\n'
...
printf '
SUCCESS: Site Intelligence v4.4.0 passed country dropdown interaction and focus-safety validation.
Repository: %s
' "$ROOT"
