...
RUNTIME_SANDBOX="$(mktemp -d "${TMPDIR:-/tmp}/scsi-v32372-runtime.XXXXXX")"
trap 'rm -rf "$RUNTIME_SANDBOX" "$BACKEND/backend"' EXIT
export SC_SI_RUNTIME_STATE_ROOT="$RUNTIME_SANDBOX"

printf '
==> Running mandatory v3.25.0 country dropdown interaction and focus-safety gate
'
"$PYTHON" "$ROOT/scripts/run_browser_gate_v32372.py" browser_country_selector_interaction_v32372.py

printf '
==> Running mandatory v3.25.0 global country selector hydration gate
'
"$PYTHON" "$ROOT/scripts/run_browser_gate_v32372.py" browser_country_selector_hydration_v32372.py

printf '\n==> Running mandatory v3.25.0 global country truth browser gate\n'
"$PYTHON" "$ROOT/scripts/run_browser_gate_v32372.py" browser_global_country_data_truth_v32372.py

rm -rf "$BACKEND/backend"
printf '
==> Running complete inherited and v3.25.0 test suite with process-isolated teardown
'
PYTHONPATH="$BACKEND" PYTEST_DISABLE_PLUGIN_AUTOLOAD=1 "$PYTHON" "$ROOT/scripts/run_v32372_test_suite.py"
printf '
==> Verifying v3.25.0 country dropdown interaction and focus safety
'
"$PYTHON" "$ROOT/scripts/validate_v32372_release.py"
grep -q 'APP_VERSION = "3.25.0"' "$BACKEND/app/version.py"
grep -q 'Version: 3.25.0' "$ROOT/wordpress-plugin/sustainable-catalyst-site-intelligence/sustainable-catalyst-site-intelligence.php"
grep -q 'const RELEASE="3.25.0"' "$BACKEND/public_app/service-worker.js"
grep -q '/public/data-truth/coverage-matrix' "$BACKEND/app/main.py"
grep -q 'data-truth-v32371.js?v=3.25.0' "$BACKEND/public_app/index.html"
...
grep -q 'function userControlFocused()' "$BACKEND/public_app/assets/browser-reliability-v3235.js"
grep -q "if(!next||next===lastRoute)return" "$BACKEND/public_app/assets/browser-reliability-v3235.js"
grep -q 'if(userControlFocused())return' "$BACKEND/public_app/assets/browser-reliability-v3235.js"
grep -q '#countrySelect{touch-action:auto;overscroll-behavior:auto;scroll-behavior:auto}' "$BACKEND/public_app/assets/app.css"
...
root=Path(sys.argv[1]);m=json.loads((root/'MANIFEST.json').read_text());assert m['release']=='3.25.0';assert m['file_count']==len(m['files'])
for e in m['files']:
...
'
"$PYTHON" "$ROOT/scripts/run_browser_gate_v32372.py" browser_complete_shell_gate_v32362.py
printf '\n==> Running mandatory v3.25.0 production-soak route and service-worker gate\n'
"$PYTHON" "$ROOT/scripts/run_browser_gate_v32372.py" browser_production_soak_gate_v32364.py
printf '\n==> Running mandatory long-page WordPress embed gate\n'
"$PYTHON" "$ROOT/scripts/run_browser_gate_v32372.py" browser_wordpress_embed_gate_v32363.py
rm -rf "$BACKEND/backend"
[[ ! -e "$BACKEND/backend" ]] || { echo 'ERROR: runtime state cleanup failed.' >&2; exit 1; }
printf '
SUCCESS: Site Intelligence v3.25.0 passed country dropdown interaction and focus-safety validation.
Repository: %s
' "$ROOT"
