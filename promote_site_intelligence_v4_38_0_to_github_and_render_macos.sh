...
for ((attempt=1; attempt<=ATTEMPTS; attempt++)); do
 ...
 if [[ "$V" == "$RELEASE" && "$RID" == "$RELEASE_ID" && "$G" == ready && ( "$C" == "$COMMIT" || "$COMMIT" == "$C"* || "$C" == "$COMMIT"* ) ]]; then
  ...
  app_ok=false; [[ "$app" == *'/app/assets/app.js?v=4.39.1'* && "$app" == *'/app/assets/live-space-observation-v4380.js?v=4.39.1'* && "$app" == *'/app/assets/iframe-navigation-v4380.css?v=4.39.1'* && "$app" == *'data-ocean-entry="hub"'* && "$app" == *'data-space-entry="hub"'* && "$iframe_css" == *'236px'* ]] && app_ok=true
  ...
  if [[ "$health_ok" == true && "$dep_ok" == true && "$sp_ok" == true && "$providers_ok" == true && "$ocean_ok" == true && "$science_ok" == true && "$app_ok" == true ]]; then
   write_receipt verified 'v4.39.1 release identity, five-lane live Space control plane, iframe navigation, Ocean/Space discovery and application assets synchronized.'
