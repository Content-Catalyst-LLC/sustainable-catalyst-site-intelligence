...
PY
if command -v node >/dev/null 2>&1; then node "$ROOT/scripts/check_javascript_v41900.js" "$BACKEND/public_app" "$ROOT/wordpress-plugin/sustainable-catalyst-site-intelligence/assets"; fi
...
if [[ "${SC_SI_RUN_BROWSER:-0}" == "1" ]]; then PYTHON="$PYTHON" "$PYTHON" "$ROOT/scripts/browser_terrestrial_ecosystems_v41900.py"; fi
echo 'SUCCESS: Site Intelligence v4.19.0 passed Terrestrial Ecosystems, Vegetation & Wildfire Intelligence validation.'
