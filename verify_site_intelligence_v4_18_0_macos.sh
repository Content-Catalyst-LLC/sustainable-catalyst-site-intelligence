...
grep -q 'APP_VERSION = "4.18.0"' "$BACKEND/app/version.py"
grep -q 'Version: 4.18.0' "$ROOT/wordpress-plugin/sustainable-catalyst-site-intelligence/sustainable-catalyst-site-intelligence.php"
...
for asset in ocean-surface-v4500 water-column-v4600 seafloor-bathymetry-v4700 underwater-observation-v4800 marine-biodiversity-v4900 ocean-missions-v41000 ocean-events-v41100 marine-human-activity-v41200 marine-pollution-v41300 coastal-change-v41400 ocean-governance-v41500 cryosphere-v41600 atmosphere-v41700 hydrology-v41800; do
...
PY
if command -v node >/dev/null 2>&1; then node "$ROOT/scripts/check_javascript_v41800.js" "$BACKEND/public_app" "$ROOT/wordpress-plugin/sustainable-catalyst-site-intelligence/assets"; fi
...
if [[ "${SC_SI_RUN_BROWSER:-0}" == "1" ]]; then PYTHON="$PYTHON" "$PYTHON" "$ROOT/scripts/browser_hydrology_v41800.py"; fi
echo 'SUCCESS: Site Intelligence v4.18.0 passed Global Hydrology, Rivers, Flood & Drought Intelligence validation.'
