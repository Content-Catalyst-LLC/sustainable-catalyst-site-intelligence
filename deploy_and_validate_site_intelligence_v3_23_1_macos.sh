...
PYTHON="$ROOT/.venv/bin/python" bash "$ROOT/verify_site_intelligence_v3_23_1_macos.sh"
printf '\n==> Running deterministic validation pass 2\n'
PYTHON="$ROOT/.venv/bin/python" bash "$ROOT/verify_site_intelligence_v3_23_1_macos.sh"
printf '\n==> Promoting the backend through GitHub and Render\n'
PYTHON="$ROOT/.venv/bin/python" bash "$ROOT/promote_site_intelligence_v3_23_1_to_github_and_render_macos.sh"
...
SUCCESS: Site Intelligence v${RELEASE} is live with production truth and workspace completion.
...
Install the WordPress ZIP only now that the live gate has verified the exact release id, Git commit, app shell, production-truth directory, active-route state runtime, cartographic workspace, local geography, and runtime health.
DONE
