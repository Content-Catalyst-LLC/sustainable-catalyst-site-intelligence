#!/usr/bin/env bash
set -euo pipefail
RELEASE="4.35.24"; RELEASE_ID="site-intelligence-v${RELEASE}"; BUNDLE="${1:-}"
fail(){ printf '\nERROR: %s\n' "$1" >&2; exit 1; }
if [[ -z "$BUNDLE" ]]; then BUNDLE="$(find "$HOME/Downloads" -maxdepth 1 -type f -name 'sustainable-catalyst-site-intelligence-v4.35.24-release-bundle*.zip' -print0 2>/dev/null | xargs -0 ls -t 2>/dev/null | head -1 || true)"; fi
...
printf '\n==> Running deterministic validation pass 1\n'; PYTHON="$ROOT/.venv/bin/python" bash "$ROOT/verify_site_intelligence_v4_35_24_macos.sh"
printf '\n==> Running deterministic validation pass 2\n'; SC_SI_SKIP_TESTS=1 SC_SI_RUN_BROWSER=1 PYTHON="$ROOT/.venv/bin/python" bash "$ROOT/verify_site_intelligence_v4_35_24_macos.sh"
printf '\n==> Promoting backend through GitHub and Render\n'; PYTHON="$ROOT/.venv/bin/python" bash "$ROOT/promote_site_intelligence_v4_35_24_to_github_and_render_macos.sh"
printf '\nSUCCESS: Site Intelligence v%s is live with Palestine-First Country Navigation Integrity, Canonical Country Identity, Palestine Data Federation, Wikimedia Knowledge Context & Semantic Truth.\nRelease id: %s\n\nWordPress plugin ZIP:\n%s\n' "$RELEASE" "$RELEASE_ID" "$WORDPRESS_ZIP"
