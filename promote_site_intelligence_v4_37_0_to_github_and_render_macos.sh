#!/usr/bin/env bash
set -euo pipefail
RELEASE="4.39.1"; RELEASE_ID="site-intelligence-v${RELEASE}"; RELEASE_TAG="v${RELEASE}"
...
for ((attempt=1; attempt<=ATTEMPTS; attempt++)); do
  ...
  if [[ "$V" == "$RELEASE" && "$RID" == "$RELEASE_ID" && "$G" == ready && ( "$C" == "$COMMIT" || "$COMMIT" == "$C"* || "$C" == "$COMMIT"* ) ]]; then
    ...
    science_ok="$(python3 -c 'import json,sys; d=json.load(sys.stdin); domains={x.get("id") for x in d.get("domains",[])}; print("true" if d.get("ok") and d.get("version")==sys.argv[1] and d.get("release_lineage")=="v4.39.1" and d.get("local_workspace_count")==8 and domains=={"earth","ocean","space"} else "false")' "$RELEASE" <<<"$science" 2>/dev/null || echo false)"
    app_ok=false; [[ "$app" == *'/app/assets/app.js?v=4.39.1'* && "$app" == *'data-ocean-entry="hub"'* && "$app" == *'data-space-entry="hub"'* && "$seafloor" == *'underwater-observation-v4800.js?v=4.39.1'* ]] && app_ok=true
    ...
    if [[ "$health_ok" == true && "$dep_ok" == true && "$uw_ok" == true && "$providers_ok" == true && "$ocean_ok" == true && "$science_ok" == true && "$app_ok" == true ]]; then
      write_receipt verified 'v4.39.1 release identity, live underwater media control plane, Ocean/Space discovery and application assets synchronized.'
