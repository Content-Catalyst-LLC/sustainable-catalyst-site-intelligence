...
RUNTIME_SANDBOX="$(mktemp -d "${TMPDIR:-/tmp}/scsi-v3232-runtime.XXXXXX")"
trap 'rm -rf "$RUNTIME_SANDBOX" "$BACKEND/backend"' EXIT
export SC_SI_RUNTIME_STATE_ROOT="$RUNTIME_SANDBOX"
rm -rf "$BACKEND/backend"

printf '\n==> Verifying v3.23.6.3 release identity and cartographic interaction contracts\n'
grep -q 'APP_VERSION = "3.23.6.3"' "$BACKEND/app/version.py"
grep -q 'Version: 3.23.6.3' "$ROOT/wordpress-plugin/sustainable-catalyst-site-intelligence/sustainable-catalyst-site-intelligence.php"
grep -q 'const RELEASE="3.23.6.3"' "$BACKEND/public_app/service-worker.js"
grep -q 'cartographic-interaction-v3232.js' "$BACKEND/public_app/index.html"
grep -q 'cartographic-interaction-v3232.css' "$BACKEND/public_app/index.html"
grep -q '/public/maps/interaction' "$BACKEND/app/main.py"
grep -q 'SCSIOverviewMapV3232' "$BACKEND/public_app/assets/app.js"
grep -q 'SCSICartographicInteractionV3232' "$BACKEND/public_app/assets/cartographic-interaction-v3232.js"
grep -q 'mapInteractionJsUrl' "$ROOT/wordpress-plugin/sustainable-catalyst-site-intelligence/sustainable-catalyst-site-intelligence.php"
...
assert manifest['release']=='3.23.6.3'; assert manifest['file_count']==len(manifest['files'])
...
if [[ "${SC_SI_SKIP_BROWSER_SMOKE:-0}" != "1" ]]; then
  printf '\n==> Running cartographic-workspace Chromium smoke test when available\n'
  "$PYTHON" "$ROOT/scripts/browser_smoke_v3230.py"
  printf '\n==> Running production-truth Chromium smoke test when available\n'
  "$PYTHON" "$ROOT/scripts/browser_smoke_v3231.py"
  printf '\n==> Running cartographic-interaction Chromium smoke test when available\n'
...
printf '\nSUCCESS: Site Intelligence v3.23.6.3 passed deterministic validation.\nRepository: %s\nRuntime sandbox: isolated and removed on exit\n' "$ROOT"
