...
printf '\n==> Verifying v3.23.4 release identity and cartographic interaction contracts\n'
grep -q 'APP_VERSION = "3.23.4"' "$BACKEND/app/version.py"
grep -q 'Version: 3.23.4' "$ROOT/wordpress-plugin/sustainable-catalyst-site-intelligence/sustainable-catalyst-site-intelligence.php"
grep -q 'const RELEASE="3.23.4"' "$BACKEND/public_app/service-worker.js"
...
assert manifest['release']=='3.23.4'; assert manifest['file_count']==len(manifest['files'])
...
printf '\nSUCCESS: Site Intelligence v3.23.4 passed deterministic validation.\nRepository: %s\nRuntime sandbox: isolated and removed on exit\n' "$ROOT"
