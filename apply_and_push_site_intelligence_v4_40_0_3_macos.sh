...
VERSION="4.40.0.3"
TAG="v4.40.0.3"
COMMIT_MSG="Site Intelligence v4.40.0.3 live intelligence ticker pace and readability repair"
...
case "$CURRENT" in
  4.40.0.2|4.40.0.3) ;;
  *) fail "expected Site Intelligence 4.40.0.1 or 4.40.0.3, found ${CURRENT:-unknown}" ;;
esac

echo "=== APPLYING v$VERSION REPAIR ==="
rsync -a --exclude='__pycache__/' --exclude='.pytest_cache/' --exclude='*.pyc' "$PATCH/" "$REPO/"

python3 "$REPO/scripts/validate_v44003_release.py"
...
if python3 -c 'import pytest' >/dev/null 2>&1; then
  (cd "$REPO" && PYTHONPATH=backend python3 -m pytest -q \
    backend/tests/test_live_intelligence_ticker_pace_v44003.py \
...
if ! git -C "$REPO" rev-parse "$TAG" >/dev/null 2>&1; then
  git -C "$REPO" tag -a "$TAG" -m "Site Intelligence v4.40.0.3 — Live Intelligence Ticker Pace & Readability Repair"
fi
