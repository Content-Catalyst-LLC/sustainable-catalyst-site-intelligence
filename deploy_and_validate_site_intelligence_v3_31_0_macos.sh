...
fail(){ printf '\nERROR: %s\n' "$1" >&2; exit 1; }
if [[ -z "$BUNDLE" ]]; then
  BUNDLE="$(find "$HOME/Downloads" -maxdepth 1 -type f -name 'sustainable-catalyst-site-intelligence-v4.11.0-release-bundle*.zip' -print0 2>/dev/null | xargs -0 ls -t 2>/dev/null | head -1 || true)"
fi
...
SC_SI_STRICT_DEPENDENCIES=1 PYTHON="$ROOT/.venv/bin/python" bash "$ROOT/verify_site_intelligence_v3_31_0_macos.sh"
printf '\n==> Running deterministic validation pass 2 (static/package assurance; browser and regression suite already passed)\n'
SC_SI_STRICT_DEPENDENCIES=1 SC_SI_SKIP_BROWSER=1 SC_SI_SKIP_TESTS=1 PYTHON="$ROOT/.venv/bin/python" bash "$ROOT/verify_site_intelligence_v3_31_0_macos.sh"
printf '\n==> Promoting the backend through GitHub and Render\n'
PYTHON="$ROOT/.venv/bin/python" bash "$ROOT/promote_site_intelligence_v3_31_0_to_github_and_render_macos.sh"
...
SUCCESS: Site Intelligence v${RELEASE} is live with Security, Observability, Performance, and Scale Assurance.
...
Install the WordPress ZIP only now that the live gate has verified the exact release, Git commit, application shell, maps, analytical workflows, institutional workspaces/review governance, briefing/story-map/publication studio, monitoring/digest/early-warning operations, research evidence/knowledge integration, comparative/scenario/model assurance, browser reliability, performance/offline contract, single-owner bootstrap contract, mutation-observer recovery contract, mandatory complete-shell browser gate, service-worker strategy, production truth, and runtime health.
DONE
