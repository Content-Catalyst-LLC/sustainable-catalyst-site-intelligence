...
printf '\n==> Running deterministic validation pass 1\n'; PYTHON="$ROOT/.venv/bin/python" bash "$ROOT/verify_site_intelligence_v4_35_11_macos.sh"
printf '\n==> Running deterministic validation pass 2\n'; SC_SI_SKIP_TESTS=1 SC_SI_RUN_BROWSER=1 PYTHON="$ROOT/.venv/bin/python" bash "$ROOT/verify_site_intelligence_v4_35_11_macos.sh"
printf '\n==> Promoting backend through GitHub and Render\n'; PYTHON="$ROOT/.venv/bin/python" bash "$ROOT/promote_site_intelligence_v4_35_11_to_github_and_render_macos.sh"
printf '\nSUCCESS: Site Intelligence v%s is live with Authoritative Connector High-Priority Workspace Connector Closure I.\nRelease id: %s\n\nWordPress plugin ZIP:\n%s\n' "$RELEASE" "$RELEASE_ID" "$WORDPRESS_ZIP"
