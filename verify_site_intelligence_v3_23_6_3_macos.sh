...
RUNTIME_SANDBOX="$(mktemp -d "${TMPDIR:-/tmp}/scsi-v32363-runtime.XXXXXX")"
trap 'rm -rf "$RUNTIME_SANDBOX" "$BACKEND/backend"' EXIT
export SC_SI_RUNTIME_STATE_ROOT="$RUNTIME_SANDBOX"
rm -rf "$BACKEND/backend"
printf '
==> Verifying v3.23.6.3 fixed application viewport and WordPress embed isolation
'
...
printf '
==> Running complete inherited and v3.23.6.3 test suite with process-isolated teardown
'
PYTHON="$PYTHON" "$PYTHON" "$ROOT/scripts/run_v32363_test_groups.py"
printf '
==> Running mandatory complete-shell Chromium browser gate
'
"$PYTHON" "$ROOT/scripts/run_browser_gate_v32363.py" browser_complete_shell_gate_v32362.py
printf '\n==> Running mandatory long-page WordPress embed gate\n'
"$PYTHON" "$ROOT/scripts/run_browser_gate_v32363.py" browser_wordpress_embed_gate_v32363.py
rm -rf "$BACKEND/backend"
[[ ! -e "$BACKEND/backend" ]] || { echo 'ERROR: runtime state cleanup failed.' >&2; exit 1; }
printf '
SUCCESS: Site Intelligence v3.23.6.3 passed fixed application viewport and WordPress embed isolation validation.
Repository: %s
' "$ROOT"
