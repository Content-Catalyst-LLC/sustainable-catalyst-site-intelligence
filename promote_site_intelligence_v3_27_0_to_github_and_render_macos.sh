...
if [[ -x "$SOURCE_ROOT/.venv/bin/python" ]]; then PYTHON="$SOURCE_ROOT/.venv/bin/python" bash "$SOURCE_ROOT/verify_site_intelligence_v3_27_0_macos.sh"; else bash "$SOURCE_ROOT/verify_site_intelligence_v3_27_0_macos.sh"; fi
...
if [[ -x "$SOURCE_ROOT/.venv/bin/python" ]]; then PYTHON="$SOURCE_ROOT/.venv/bin/python" bash "$CLONE_ROOT/verify_site_intelligence_v3_27_0_macos.sh"; else bash "$CLONE_ROOT/verify_site_intelligence_v3_27_0_macos.sh"; fi
cd "$CLONE_ROOT"
...
if ! git diff --cached --quiet; then
  ...
  git commit -m "Site Intelligence v${RELEASE} — research evidence and knowledge integration"
fi
...
for ((attempt=1; attempt<=ATTEMPTS; attempt++)); do
  ...
  if [[ -n "$payload" ]]; then
    ...
    if [[ "$OBSERVED_VERSION" == "$RELEASE" && "$OBSERVED_RELEASE_ID" == "$RELEASE_ID" && "$OBSERVED_GATE" == "ready" && ( "$OBSERVED_COMMIT" == "$COMMIT" || "$COMMIT" == "$OBSERVED_COMMIT"* || "$OBSERVED_COMMIT" == "$COMMIT"* ) ]]; then
      ...
      if [[ "$app_ready" == true && "$selector_ready" == true && "$selector_interaction_ready" == true && "$engine_ready" == true && "$world_ready" == true && "$workspace_ready" == true && "$interaction_ready" == true && "$data_truth_ready" == true && "$country_truth_ready" == true && "$country_matrix_ready" == true && "$record_truth_ready" == true && "$control_plane_ready" == true && "$unified_state_ready" == true && "$assurance_ready" == true && "$research_integration_ready" == true && "$analytical_ready" == true && "$browser_reliability_ready" == true && "$performance_offline_ready" == true && "$startup_stability_ready" == true && "$bootstrap_recovery_ready" == true && "$mutation_recovery_ready" == true && "$embed_isolation_ready" == true && "$truth_ready" == true && "$health_ready" == true ]]; then
        write_receipt "verified" "GitHub, Render, the live app shell, self-hosted map engine, cartographic workspace, interaction controls, data-truth directory, global country truth, hydrated global selector, focus-safe dropdown interaction, coverage matrix, record provenance and indicator truth, Unified Analytical Workspace and Cross-View State, comparative/scenario/model assurance, research evidence/knowledge integration with preview-only human-confirmed handoffs, schema drift, source incidents, coverage monitoring, cross-workspace truth, five-workflow analytical directory, browser reliability contract, performance/offline contract, production-soak startup-stability contract, single-owner bootstrap contract, mutation-observer recovery contract, fixed WordPress embed-isolation contract, complete-shell browser gate, service-worker strategy, production-truth directory, runtime health, and WordPress installation gate are synchronized."
        printf 'Render release gate and live map runtime are ready for v%s at commit %s.
' "$OBSERVED_VERSION" "${OBSERVED_COMMIT:0:12}"
        printf '
SUCCESS: Site Intelligence v4.12.0 is live with Research Evidence and Knowledge Integration.
