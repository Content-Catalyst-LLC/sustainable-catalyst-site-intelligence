...
"$PYTHON" "$ROOT/scripts/validate_v41000_release.py"
...
PY
if command -v node >/dev/null 2>&1; then node "$ROOT/scripts/check_javascript_v41000.js" "$BACKEND/public_app" "$ROOT/wordpress-plugin/sustainable-catalyst-site-intelligence/assets"; fi
...
echo 'SUCCESS: Site Intelligence v4.14.0 passed Ocean Missions, Vehicles & Observatory Network validation.'
