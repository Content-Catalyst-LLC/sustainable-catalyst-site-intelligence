...
if [[ -x "$SOURCE_ROOT/.venv/bin/python" ]]; then PYTHON="$SOURCE_ROOT/.venv/bin/python" bash "$SOURCE_ROOT/verify_site_intelligence_v3_24_0_macos.sh"; else bash "$SOURCE_ROOT/verify_site_intelligence_v3_24_0_macos.sh"; fi
...
if [[ -x "$SOURCE_ROOT/.venv/bin/python" ]]; then PYTHON="$SOURCE_ROOT/.venv/bin/python" bash "$CLONE_ROOT/verify_site_intelligence_v3_24_0_macos.sh"; else bash "$CLONE_ROOT/verify_site_intelligence_v3_24_0_macos.sh"; fi
cd "$CLONE_ROOT"
...
if ! git diff --cached --quiet; then
  ...
  git commit -m "Site Intelligence v${RELEASE} — global data truth control plane"
fi
...
for ((attempt=1; attempt<=ATTEMPTS; attempt++)); do
  ...
  if [[ -n "$payload" ]]; then
    ...
    if [[ "$OBSERVED_VERSION" == "$RELEASE" && "$OBSERVED_RELEASE_ID" == "$RELEASE_ID" && "$OBSERVED_GATE" == "ready" && ( "$OBSERVED_COMMIT" == "$COMMIT" || "$COMMIT" == "$OBSERVED_COMMIT"* || "$OBSERVED_COMMIT" == "$COMMIT"* ) ]]; then
      ...
      app_ready=false; selector_ready=false; selector_interaction_ready=false; engine_ready=false; world_ready=false; workspace_ready=false; interaction_ready=false; data_truth_ready=false; country_truth_ready=false; country_matrix_ready=false; record_truth_ready=false; control_plane_ready=false; analytical_ready=false; browser_reliability_ready=false; performance_offline_ready=false; startup_stability_ready=false; bootstrap_recovery_ready=false; mutation_recovery_ready=false; embed_isolation_ready=false; truth_ready=false; health_ready=false
      [[ "$app_html" == *'data-scsi-release="4.17.0"'* && "$app_html" == *'vector-cartography-v3230.js'* && "$app_html" == *'cartographic-workspace-v3230.js'* && "$app_html" == *'cartographic-interaction-v3232.js'* && "$app_html" == *'data-truth-control-plane-v3240.js?v=4.17.0'* ]] && app_ready=true
