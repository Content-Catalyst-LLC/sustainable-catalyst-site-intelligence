...
RUNTIME_SANDBOX="$(mktemp -d "${TMPDIR:-/tmp}/scsi-v32361-runtime.XXXXXX")"
trap 'rm -rf "$RUNTIME_SANDBOX" "$BACKEND/backend"' EXIT
export SC_SI_RUNTIME_STATE_ROOT="$RUNTIME_SANDBOX"
rm -rf "$BACKEND/backend"

printf '\n==> Verifying v3.23.6.3 single-owner bootstrap and loading recovery\n'
"$PYTHON" "$ROOT/scripts/validate_v32361_release.py"
...
grep -q 'bootstrap-v32361.js' "$BACKEND/public_app/index.html"
grep -q '/public/bootstrap-recovery' "$BACKEND/app/main.py"
[[ "$(grep -RIl 'serviceWorker.register' "$BACKEND/public_app/assets" | wc -l | tr -d ' ')" == "1" ]]
grep -q 'serviceWorker.register' "$BACKEND/public_app/assets/bootstrap-v32361.js"
...
root=Path(sys.argv[1]);m=json.loads((root/'MANIFEST.json').read_text());assert m['release']=='3.23.6.3';assert m['file_count']==len(m['files'])
for e in m['files']:
...
printf '\n==> Running complete inherited and v3.23.6.3 test suite\n'
(
  cd "$BACKEND"
...
if [[ "${SC_SI_SKIP_BROWSER_SMOKE:-0}" != "1" ]]; then
  ...
  for smoke in browser_smoke_v3229.py browser_smoke_v3230.py browser_smoke_v3231.py browser_smoke_v3232.py browser_smoke_v3233.py browser_smoke_v3234.py browser_smoke_v3235.py browser_smoke_v3236.py browser_smoke_v32361.py browser_full_startup_v32361.py; do
    [[ -f "$ROOT/scripts/$smoke" ]] && "$PYTHON" "$ROOT/scripts/$smoke"
  done
fi
rm -rf "$BACKEND/backend"
[[ ! -e "$BACKEND/backend" ]] || { echo 'ERROR: runtime state cleanup failed.' >&2; exit 1; }
printf '\nSUCCESS: Site Intelligence v3.23.6.3 passed single-owner bootstrap and loading recovery validation.\nRepository: %s\n' "$ROOT"
