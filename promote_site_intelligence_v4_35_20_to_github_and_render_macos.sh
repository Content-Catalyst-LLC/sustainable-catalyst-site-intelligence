...
if [[ -x "$SOURCE_ROOT/.venv/bin/python" ]]; then SC_SI_STRICT_DEPENDENCIES=1 SC_SI_SKIP_BROWSER=1 PYTHON="$SOURCE_ROOT/.venv/bin/python" bash "$SOURCE_ROOT/verify_site_intelligence_v4_35_20_macos.sh"; else SC_SI_SKIP_BROWSER=1 bash "$SOURCE_ROOT/verify_site_intelligence_v4_35_20_macos.sh"; fi
...
if [[ -x "$SOURCE_ROOT/.venv/bin/python" ]]; then SC_SI_STRICT_DEPENDENCIES=1 SC_SI_SKIP_BROWSER=1 PYTHON="$SOURCE_ROOT/.venv/bin/python" bash "$CLONE_ROOT/verify_site_intelligence_v4_35_20_macos.sh"; else SC_SI_SKIP_BROWSER=1 bash "$CLONE_ROOT/verify_site_intelligence_v4_35_20_macos.sh"; fi
cd "$CLONE_ROOT"
...
for ((attempt=1; attempt<=ATTEMPTS; attempt++)); do
  ...
  if [[ -n "$gate" ]]; then
    ...
    if [[ "$OBSERVED_VERSION" == "$RELEASE" && "$OBSERVED_RELEASE_ID" == "$RELEASE_ID" && "$OBSERVED_GATE" == "ready" && ( "$OBSERVED_COMMIT" == "$COMMIT" || "$COMMIT" == "$OBSERVED_COMMIT"* || "$OBSERVED_COMMIT" == "$COMMIT"* ) ]]; then
      ...
      printf 'Release verification: health=%s runtime=%s deployment=%s routes=%s connectors=%s evidence=%s workspace-evidence=%s production-audit=%s source-policy=%s credential-control=%s resilience=%s workspace-browser=%s production-soak=%s semantic-truth=%s country-linked-records=%s app=%s\n' "$health_ready" "$runtime_ready" "$deployment_ready" "$v4_ready" "$connector_ready" "$evidence_ready" "$workspace_evidence_ready" "$production_audit_ready" "$source_health_ready" "$credential_ready" "$external_resilience_ready" "$workspace_browser_ready" "$production_soak_ready" "$evidence_presentation_ready" "$country_linked_records_ready" "$app_ready"
      if [[ "$health_ready" == true && "$runtime_ready" == true && "$deployment_ready" == true && "$v4_ready" == true && "$connector_ready" == true && "$evidence_ready" == true && "$workspace_evidence_ready" == true && "$production_audit_ready" == true && "$source_health_ready" == true && "$credential_ready" == true && "$external_resilience_ready" == true && "$workspace_browser_ready" == true && "$production_soak_ready" == true && "$evidence_presentation_ready" == true && "$country_linked_records_ready" == true && "$app_ready" == true ]]; then
        ...
        printf '\nSUCCESS: Site Intelligence v%s is live with Country-Linked Record Recovery, Semantic Truth & Simply Works Reliability at commit %s.\nDeployment receipt: %s\nRollback tag: %s (%s)\n' "$RELEASE" "${OBSERVED_COMMIT:0:12}" "$DEPLOYMENT_RECEIPT" "$ROLLBACK_TAG" "${PREVIOUS_COMMIT:0:12}"
        exit 0
      fi
    fi
  fi
