...
def test_v101_page_builder_includes_copy_polish_status():
    data = public_page_builder(Settings(version='3.22.0'))
    assert data['version'] == '3.22.0'
...
def test_v101_visual_qa_payload_has_public_page_copy():
    data = public_dashboard_visual_qa(Settings(version='3.22.0'))
...
def test_v101_module_catalog_includes_public_release_qa_module():
    modules = module_manager(Settings(version='3.22.0'))['modules']
