...
def test_public_data_truth_directory_discloses_non_live_states_and_boundaries():
    ...
    assert payload["version"] == "3.23.6.4"
    assert payload["release_id"] == "site-intelligence-v3.23.6.4"
...
def test_registry_has_canonical_truth_metadata_for_every_source():
    ...
    assert registry["version"] == "3.23.6.4"
...
def test_app_shell_loads_data_truth_inside_application_before_production_truth():
    ...
    assert '/app/assets/data-truth-v3233.css?v=3.23.6.4' in html
    assert '/app/assets/data-truth-v3233.js?v=3.23.6.4' in html
    assert html.index("cartographic-interaction-v3232.js") < html.index("data-truth-v3233.js") < html.index("production-truth-v3231.js")
    js = read("backend/public_app/assets/data-truth-v3233.js")
    for token in ("dataTruthToggle", "dataTruthPanel", "stale_marker_required", "Cached, historical, demonstration", "SCSIDataTruthV3233"):
        assert token in js
...
def test_runtime_health_offline_shell_and_wordpress_package_include_truth_assets_without_host_execution():
    ...
    assert "/app/assets/data-truth-v3233.js" in paths
    assert "/app/assets/data-truth-v3233.css" in paths
    assert "/public/data-truth" in endpoints
    worker = read("backend/public_app/service-worker.js")
    assert "data-truth-v3233.js" in worker and "data-truth-v3233.css" in worker
    php = read("wordpress-plugin/sustainable-catalyst-site-intelligence/sustainable-catalyst-site-intelligence.php")
    assert "Version: 3.23.6.4" in php
    assert "dataTruthCssUrl" in php and "dataTruthJsUrl" in php
    assert "wp_enqueue_script('scsi-data-truth'" not in php
    assert (ROOT / "wordpress-plugin/sustainable-catalyst-site-intelligence/assets/data-truth-v3233.js").is_file()
    assert (ROOT / "wordpress-plugin/sustainable-catalyst-site-intelligence/assets/data-truth-v3233.css").is_file()
