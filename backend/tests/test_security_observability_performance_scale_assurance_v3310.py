...
def test_production_assurance_public_contract():
    ...
    d=r.json(); assert d["version"]=="4.11.0" and d["contract"]=="security-observability-performance-scale-assurance"
...
def test_post_deploy_smoke_is_preview_only():
    d=CLIENT.post("/public/production-assurance/post-deploy/preview",json={"release":"4.11.0","commit":"abc123"}).json(); assert d["preview"]["network_requests_performed"] is False and d["preview"]["deployment_mutated"] is False and len(d["preview_sha256"])==64
...
def test_assets_and_service_worker_ship_v3310_layer():
    ...
    assert "security-performance-v3310.js?v=4.11.0" in html and "security-performance-v3310.js" in sw and "SCSIProductionAssuranceV3310" in js
