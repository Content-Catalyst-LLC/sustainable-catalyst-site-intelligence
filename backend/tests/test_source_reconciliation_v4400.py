...
def test_reconciliation_does_not_rewrite_historical_origin_metadata() -> None:
    payload = json.loads((_data_root() / "analytical_workspace_policy_v3234.json").read_text(encoding="utf-8"))
    # `version` is the active runtime compatibility marker.  The release_id
    # remains the historical origin marker for this policy family.
