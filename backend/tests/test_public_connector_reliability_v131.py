...
def test_connector_reliability_payload_is_public_safe():
    data = public_connector_reliability(Settings(version="3.6.1", external_live=True, public_connector_live_checks=True))
    assert data["ok"] is True
    assert data["version_scope"] == "v3.6.1"
...
def test_connector_status_now_includes_reliability_counts():
    data = public_connector_status(Settings(version="3.6.1", external_live=True, public_connector_live_checks=True))
    assert data["version_scope"] == "v3.6.1"
...
def test_status_polish_payload_contains_display_guidance_and_hidden_fields():
    data = public_connector_status_polish(Settings(version="3.6.1"))
    assert data["ok"] is True
...
def test_admin_diagnostics_includes_recovery_queue_without_secrets():
    settings = Settings(version="3.6.1", eia_api_key="secret-value", epa_aqs_key="secret-value", epa_aqs_email="ops@example.com")
