...
def test_connector_reliability_payload_is_public_safe():
    data = public_connector_reliability(Settings(version="1.6.0"))
...
def test_status_polish_payload_contains_display_guidance_and_hidden_fields():
    data = public_connector_status_polish(Settings(version="1.6.0"))
    assert data["ok"] is True
...
def test_admin_diagnostics_includes_recovery_queue_without_secrets():
    settings = Settings(version="1.6.0", eia_api_key="secret-value", epa_aqs_key="secret-value", epa_aqs_email="ops@example.com")
