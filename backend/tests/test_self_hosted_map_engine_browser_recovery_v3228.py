...
def test_local_world_boundary_asset_is_real_and_publicly_served():
    path = ROOT / "backend/public_app/assets/world-cartography-v3229.geojson"
    payload = json.loads(path.read_text(encoding="utf-8"))
    assert payload["type"] == "FeatureCollection"
    assert len(payload["features"]) >= 170
    response = client.get("/app/assets/world-cartography-v3229.geojson")
...
def test_application_loads_engine_before_all_map_modules():
    ...
    engine = html.index("/app/assets/vector-cartography-v3229.js?v=3.22.9")
    recovery = html.index("/app/assets/service-recovery-v3224.js?v=3.22.9")
    runtime = html.index("/app/assets/runtime-v3229.js?v=3.22.9")
    application = html.index("/app/assets/app.js?v=3.22.9")
...
def test_service_worker_uses_network_first_for_versioned_map_assets():
    ...
    assert "world-cartography-v3229.geojson" in worker
...
def test_wordpress_embeds_force_release_parity_and_bundle_same_engine():
    ...
    assert (ROOT / "wordpress-plugin/sustainable-catalyst-site-intelligence/assets/world-cartography-v3229.geojson").is_file()
...
def test_live_promotion_verifies_rendered_geography_and_runtime_health():
    ...
    assert "/app/assets/vector-cartography-v3229.js" in script
    assert "/app/assets/world-cartography-v3229.geojson" in script
...
def test_world_boundary_metadata_and_geometry_contract():
    payload = json.loads((ROOT / "backend/public_app/assets/world-cartography-v3229.geojson").read_text(encoding="utf-8"))
    assert payload["type"] == "FeatureCollection"
    assert len(payload["features"]) >= 170
