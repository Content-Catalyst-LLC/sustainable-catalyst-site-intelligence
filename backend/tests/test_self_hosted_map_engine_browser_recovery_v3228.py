...
def test_self_hosted_engine_replaces_false_positive_tile_stub():
    engine = read("backend/public_app/assets/map-engine-v3228.js")
    assert "class SelfHostedTileLayer" in engine
    assert 'document.createElement("img")' in engine
    assert "world-boundaries-v3228.geojson" in engine
    assert "__scsiSelfHosted: true" in engine
    assert "self-hosted-local-vector" in engine
...
def test_local_world_boundary_asset_is_real_and_publicly_served():
    path = ROOT / "backend/public_app/assets/world-boundaries-v3228.geojson"
    payload = json.loads(path.read_text(encoding="utf-8"))
    assert payload["type"] == "FeatureCollection"
    assert len(payload["features"]) >= 170
    response = client.get("/app/assets/world-boundaries-v3228.geojson")
...
def test_application_loads_engine_before_all_map_modules():
    ...
    engine = html.index("/app/assets/map-engine-v3228.js?v=3.22.8")
    recovery = html.index("/app/assets/service-recovery-v3224.js?v=3.22.8")
    runtime = html.index("/app/assets/runtime-v3228.js?v=3.22.8")
    application = html.index("/app/assets/app.js?v=3.22.8")
    assert engine < recovery < runtime < application
    assert 'data-map-runtime="self-hosted-map-engine"' in html


def test_runtime_health_separates_optional_imagery_from_application_health():
    runtime = read("backend/public_app/assets/runtime-v3228.js")
    engine = read("backend/public_app/assets/map-engine-v3228.js")
...
def test_service_worker_uses_network_first_for_versioned_map_assets():
    ...
    assert "world-boundaries-v3228.geojson" in worker
...
def test_wordpress_embeds_force_release_parity_and_bundle_same_engine():
    ...
    assert "world-boundaries-v3228.geojson" in php
    ...
    assert (ROOT / "wordpress-plugin/sustainable-catalyst-site-intelligence/assets/world-boundaries-v3228.geojson").is_file()
...
def test_runtime_contract_reports_local_vector_basemap():
    ...
    assert "/app/assets/map-engine-v3228.js" in paths
    assert "/app/assets/world-boundaries-v3228.geojson" in paths
    assert payload["recovery_policy"]["map_engine"].startswith("Use the self-hosted map engine")

def test_current_release_scripts_are_unambiguous():
    assert (ROOT / "verify_site_intelligence_v3_22_8_macos.sh").is_file()
    assert (ROOT / "promote_site_intelligence_v3_22_8_to_github_and_render_macos.sh").is_file()
...
def test_current_release_documentation_is_present():
    for name in (
        "RELEASE_NOTES_SITE_INTELLIGENCE_V3228.md",
        "SITE_INTELLIGENCE_V3228_MAP_ENGINE_AUDIT.md",
        "SITE_INTELLIGENCE_V3228_INSTALL_AND_TEST.md",
        "SITE_INTELLIGENCE_V3228_TERMINAL_COMMANDS.txt",
        "SITE_INTELLIGENCE_V3228_BUILD_VALIDATION.txt",
    ):
        assert (ROOT / name).is_file(), name


def test_live_promotion_verifies_rendered_geography_and_runtime_health():
    script = (ROOT / "promote_site_intelligence_v3_22_8_to_github_and_render_macos.sh").read_text(encoding="utf-8")
    assert "/app/?release=${RELEASE}" in script
    assert "/app/assets/map-engine-v3228.js" in script
    assert "/app/assets/world-boundaries-v3228.geojson" in script
...
def test_world_boundary_metadata_and_geometry_contract():
    payload = json.loads((ROOT / "backend/public_app/assets/world-boundaries-v3228.geojson").read_text(encoding="utf-8"))
    assert payload["type"] == "FeatureCollection"
    assert len(payload["features"]) >= 170
