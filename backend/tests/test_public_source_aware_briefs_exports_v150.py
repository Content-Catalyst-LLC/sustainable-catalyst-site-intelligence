...
def test_source_aware_brief_directory_is_public_safe():
    data = public_source_aware_brief_directory(Settings(version="3.16.0"))
...
def test_source_aware_brief_has_citations_and_boundaries():
    data = public_source_aware_brief("indicator", Settings(version="3.16.0"))
    assert data["ok"] is True
...
def test_dashboard_export_visual_qa():
    data = public_dashboard_export_visual_qa(Settings(version="3.16.0"))
    assert data["ok"] is True
