...
def test_release_identity_and_workspace_assets_are_current():
    assert 'APP_VERSION = "4.35.18"' in read("backend/app/version.py")
    html = read("backend/public_app/index.html")
    assert 'data-scsi-release="4.35.18"' in html
    assert "/app/assets/cartographic-workspace-v3230.css?v=4.35.18" in html
    assert "/app/assets/cartographic-workspace-v3230.js?v=4.35.18" in html
...
def test_current_promotion_gate_and_browser_smoke_are_declared():
    ...
    assert 'RELEASE="4.35.18"' in promote
