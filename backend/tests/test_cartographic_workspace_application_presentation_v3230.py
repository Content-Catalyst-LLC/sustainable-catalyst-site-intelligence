...
def test_release_identity_and_workspace_assets_are_current():
    assert 'APP_VERSION = "4.36.1"' in read("backend/app/version.py")
    html = read("backend/public_app/index.html")
    assert 'data-scsi-release="4.36.1"' in html
    assert "/app/assets/cartographic-workspace-v3230.css?v=4.36.1" in html
    assert "/app/assets/cartographic-workspace-v3230.js?v=4.36.1" in html
...
def test_service_worker_caches_current_presentation_assets():
    ...
    assert 'const RELEASE="4.36.1"' in worker
    for name in (
        "cartographic-workspace-v3230.js",
        "cartographic-workspace-v3230.css",
        "vector-cartography-v3230.js",
...
def test_current_promotion_gate_and_browser_smoke_are_declared():
    ...
    assert 'RELEASE="4.36.1"' in promote
