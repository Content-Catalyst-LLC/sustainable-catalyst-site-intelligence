...
def test_runtime_assets_load_between_map_reliability_and_application():
    ...
    assert "/app/assets/runtime-v3224.css" in html
...
def test_runtime_health_tray_has_rescan_copy_and_fault_capture():
    ...
    css = (ROOT / "backend/public_app/assets/runtime-v3224.css").read_text(encoding="utf-8")
...
def test_map_runtime_degrades_to_grid_when_openstreetmap_fails():
    ...
    assert 'container.dataset.scsiMapMode = "grid-overlay"' in js
    assert "tilePane.style.opacity = \"0\"" in js
    assert ".scsi-map-grid-overlay" in css
...
def test_runtime_assets_are_offline_cached_and_release_aligned():
    ...
    assert 'const RELEASE="3.22.4"' in worker
