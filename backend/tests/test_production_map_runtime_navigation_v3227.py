...
def test_map_startup_is_entirely_first_party():
    html = read("backend/public_app/index.html")
    runtime = read("backend/public_app/assets/map-engine-v3228.js")
    assert "unpkg.com/leaflet" not in html
    assert "leaflet@1.9.4/dist/leaflet.js" not in html
    assert html.index("map-engine-v3228.js") < html.index('src="/app/assets/app.js?v=3.22.8"')
    assert '__scsiFirstParty: true' in runtime
    assert 'mode: "self-hosted-map-engine"' in runtime
...
def test_runtime_contract_reports_first_party_map_bootstrap():
    ...
    assert data["recovery_policy"]["map_engine"] == "Use the self-hosted map engine with local Natural Earth country boundaries."
...
def test_wordpress_uses_the_same_first_party_runtime_before_map_features():
    ...
    map_js = read("wordpress-plugin/sustainable-catalyst-site-intelligence/assets/map-engine-v3228.js")
...
def test_release_and_offline_shell_are_aligned():
    ...
    assert 'data-scsi-release="3.22.8"' in html
