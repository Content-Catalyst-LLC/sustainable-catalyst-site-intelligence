...
def test_public_experience_endpoints():
    ...
    assert profile.json()["application_version"] == "3.22.7"
...
def test_public_html_defers_scripts_and_does_not_eager_load_capture_library():
    ...
    assert 'src="/app/assets/map-fallback-v3224.js?v=3.22.7" defer' in html
    assert html.index('src="/app/assets/map-fallback-v3224.js?v=3.22.7" defer') < html.index('src="/app/assets/app.js" defer')
...
def test_application_version_is_aligned_across_frontend_and_backend():
    ...
    assert 'APP_VERSION="3.22.7"' in js
