...
def test_public_experience_endpoints():
    ...
    assert profile.json()["application_version"] == "3.23.6.1"
...
def test_public_html_defers_scripts_and_does_not_eager_load_capture_library():
    html = (ROOT / "backend/public_app/index.html").read_text(encoding="utf-8")
    assert 'src="/app/assets/app.js?v=3.23.6.1" defer' in html
    assert 'src="/app/assets/vector-cartography-v3230.js?v=3.23.6.1" defer' in html
    assert html.index('src="/app/assets/vector-cartography-v3230.js?v=3.23.6.1" defer') < html.index('src="/app/assets/app.js?v=3.23.6.1" defer')
...
def test_application_version_is_aligned_across_frontend_and_backend():
    ...
    assert 'APP_VERSION="3.23.6.1"' in js
    assert 'APP_VERSION = "3.23.6.1"' in version
