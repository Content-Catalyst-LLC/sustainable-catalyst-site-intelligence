...
def test_public_experience_endpoints():
    ...
    assert profile.json()["application_version"] == "4.17.0"
...
def test_public_html_defers_scripts_and_does_not_eager_load_capture_library():
    html = (ROOT / "backend/public_app/index.html").read_text(encoding="utf-8")
    assert 'src="/app/assets/app.js?v=4.17.0" defer' in html
    assert 'src="/app/assets/vector-cartography-v3230.js?v=4.17.0" defer' in html
    assert html.index('src="/app/assets/vector-cartography-v3230.js?v=4.17.0" defer') < html.index('src="/app/assets/app.js?v=4.17.0" defer')
...
def test_application_version_is_aligned_across_frontend_and_backend():
    ...
    assert 'APP_VERSION="4.17.0"' in js
    assert 'APP_VERSION = "4.17.0"' in version
