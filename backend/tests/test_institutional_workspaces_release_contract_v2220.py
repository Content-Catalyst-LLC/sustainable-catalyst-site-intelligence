...
def test_release_contract_markers():
    checks = {
        ...
        "backend/app/institutional_workspaces_v2220.py": ['RELEASE_VERSION = "3.6.1"', "def create_workspace(", "def add_member(", "def review_evidence(", "def export_workspace("],
        "backend/public_app/index.html": ['data-route="workspaces"', 'id="institutionalWorkspaceStudio"'],
        "wordpress-plugin/sustainable-catalyst-site-intelligence/sustainable-catalyst-site-intelligence.php": ['Version: 3.6.1', 'sc_public_institutional_workspaces', 'sc_institutional_workspaces_control_center'],
...
def test_service_worker_and_app_include_workspace_assets():
    ...
    assert 'const RELEASE="3.6.1"' in sw
