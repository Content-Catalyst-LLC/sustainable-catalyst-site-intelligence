...
def test_runtime_receipt_has_release_identity(monkeypatch):
    monkeypatch.setenv("SC_SI_RELEASE_ID", "site-intelligence-v4.35.23")
    data = public_deployment_receipt()
    assert data["version"] == "4.35.23"
    assert data["release_id"] == "site-intelligence-v4.35.23"
    assert len(data["receipt_fingerprint"]) == 24


def test_release_gate_checks_expected_release_id(monkeypatch):
    monkeypatch.setenv("SC_SI_RELEASE_ID", "site-intelligence-v4.35.23")
    ready = build_release_gate(plugin_version="4.35.23", expected_release_id="site-intelligence-v4.35.23")
    blocked = build_release_gate(plugin_version="4.35.23", expected_release_id="wrong-release")
...
def test_deployment_receipt_endpoint_is_uncacheable():
    ...
    assert response.json()["version"] == "4.35.23"
...
def test_render_blueprint_declares_release_and_runtime_identity():
    ...
    assert "site-intelligence-v4.35.23" in text
...
def test_wordpress_requests_release_id_and_reads_receipt():
    ...
    assert "const RELEASE_ID = 'site-intelligence-v4.35.23';" in text
...
def test_build_info_and_status_publish_release_id(monkeypatch):
    ...
    assert build["release_id"] == "site-intelligence-v4.35.23"
    assert build["deployment"]["release_id"] == "site-intelligence-v4.35.23"
    assert status["release_id"] == "site-intelligence-v4.35.23"
