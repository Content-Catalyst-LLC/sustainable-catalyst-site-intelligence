...
def test_backend_and_plugin_report_v1201():
    ...
    assert root.json()["version"] == "1.22.0"
    assert build.status_code == 200
    assert build.json()["backend_version"] == "1.22.0"
    assert build.json()["expected_wordpress_plugin_version"] == "1.22.0"
...
def test_wordpress_cache_reliability_contract():
    ...
    assert "const VERSION = '1.22.0';" in php
