...
def test_backend_and_plugin_report_v1201():
    ...
    assert root.json()["version"] == "1.20.1"
    assert build.status_code == 200
    assert build.json()["backend_version"] == "1.20.1"
    assert build.json()["expected_wordpress_plugin_version"] == "1.20.1"


def test_wordpress_cache_reliability_contract():
    php = PLUGIN.read_text(encoding="utf-8")

    assert "Version: 1.20.1" in php
    assert "const VERSION = '1.20.1';" in php
