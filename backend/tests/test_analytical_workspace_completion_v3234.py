...
def test_directory_declares_five_operational_exportable_workflows():
    ...
    assert payload["ok"] is True and payload["version"]=="4.20.0"
    assert payload["release_id"]=="site-intelligence-v4.20.0"
...
def test_app_shell_and_service_worker_package_workflow_assets_inside_app_only():
    ...
    assert "/app/assets/analytical-workspaces-v3234.css?v=4.20.0" in html
    assert "/app/assets/analytical-workspaces-v3234.js?v=4.20.0" in html
    ...
    assert "analyticalWorkspacesJsUrl" in php and "Version: 4.20.0" in php
