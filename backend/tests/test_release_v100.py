...
def test_v100_release_status_requires_token_in_production():
    ...
    data = response.json()
    assert data['version'] == '2.23.0'
    assert data['release_stage'] == 'v2.23.0_typed_cross_platform_intelligence_workflows'
...
def test_v100_module_catalog_includes_release_module():
    modules = module_manager(Settings(version='2.23.0'))['modules']
