...
def test_v100_release_status_requires_token_in_production():
    ...
    data = response.json()
    assert data['version'] == '2.18.0'
    assert data['release_stage'] == 'v2.18.0_evidence_synthesis_claims_contradiction_review'
...
def test_v100_module_catalog_includes_release_module():
    modules = module_manager(Settings(version='2.18.0'))['modules']
