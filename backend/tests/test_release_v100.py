...
def test_v100_release_public_summary_endpoint_is_public_safe():
    ...
    assert data['recommended_page']['shortcode'] == '[sc_site_intelligence_public_flagship]'
...
def test_v100_release_status_requires_token_in_production():
    ...
    data = response.json()
    assert data['version'] == '1.15.2'
    assert data['release_stage'] == 'v1.15.2_public_source_status_release'
    assert data['public_shortcode'] == '[sc_site_intelligence_public_flagship]'
...
def test_v100_release_helpers_include_launch_metadata_and_smoke_checks():
    ...
    assert any('/public/climate-energy-summary' == item['path'] for item in smoke['checks'])
...
def test_v100_module_catalog_includes_release_module():
    modules = module_manager(Settings(version='1.15.2'))['modules']
