...
def test_v100_release_status_requires_token_in_production():
    ...
    data = response.json()
    assert data['version'] == '3.1.3'
    assert data['release_stage'] == 'v3.1.3_security_privacy_governance_production_scale'
...
def test_v100_module_catalog_includes_release_module():
    modules = module_manager(Settings(version='3.1.3'))['modules']
