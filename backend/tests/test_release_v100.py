...
def test_v100_release_status_requires_token_in_production():
    ...
    data = response.json()
    assert data['version'] == '3.6.2'
    assert data['release_stage'] == 'v3.6.2_security_privacy_governance_production_scale'
...
def test_v100_module_catalog_includes_release_module():
    modules = module_manager(Settings(version='3.6.2'))['modules']
