...
def packet():
    sections=['identity', 'technologies_and_resources', 'global_context', 'provenance', 'review']
    ...
    return {"schema":"sc-energy-runtime-handoff/1.0","version":"1.2.0","packet":{"handoff_id":"es-test","source":{"product":"Library","subsystem":"Energy Systems Intelligence","version":"1.2.0"},"target":{"key":TARGET_KEY,"product":'Site Intelligence',"consumer_contract":CONSUMER_CONTRACT},"contract_refs":["integrated-energy-study-contract"],"payload":payload}}

def test_framework_contract():
    x=framework(); assert x["ok"] is True; assert x["target_key"]=='site-intelligence'; assert x["consumer_version"]=='4.40.0'; assert x["capabilities"]["automatic_execution"] is False; assert x["capabilities"]["persistence"] is False
