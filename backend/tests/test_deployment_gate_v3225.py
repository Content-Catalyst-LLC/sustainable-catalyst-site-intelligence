...
def test_release_gate_allows_matching_local_validation(monkeypatch):
    ...
    data = build_release_gate(plugin_version="4.25.0")
    assert data["backend_version"] == "4.25.0"
...
def test_release_gate_verifies_render_branch_and_commit(monkeypatch):
    ...
    monkeypatch.setenv("SC_SI_RELEASE_CHANNEL", "production")
    data = build_release_gate(plugin_version="4.25.0", expected_commit="0123456789ab")
    assert data["install_allowed"] is True
...
def test_release_gate_endpoint_is_uncacheable_and_public():
    response = client.get("/public/release-gate", params={"plugin_version": "4.25.0"})
    assert response.status_code == 200
    assert response.json()["version"] == "4.25.0"
    assert response.headers["cache-control"] == "no-cache, no-store, must-revalidate"
    assert response.headers["x-sc-release-gate"] == "v4.25.0"
