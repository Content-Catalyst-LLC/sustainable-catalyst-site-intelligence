...
# v1.7.0 connector reliability, freshness, and schema-validation layer.
import json
import threading
import time
...
def _probe_connector(item: Dict[str, Any], settings: Settings, *, force: bool = False) -> Dict[str, Any]:
    ...
    for attempt in range(1, attempts + 1):
        try:
            req = Request(item["base_url"] + item["health_path"], headers={"User-Agent": "SustainableCatalystSiteIntelligence/1.7.0", "Accept": "application/json,text/csv,*/*"})
            with urlopen(req, timeout=settings.external_request_timeout_seconds) as resp:
...
def freshness_policy() -> Dict[str, Any]:
    ...
    return {"ok": True, "generated_at": _now(), "version_scope": "v1.7.0", "title": "Sustainable Development Freshness Policy", "classes": FRESHNESS_CLASSES, "thresholds": rows, "public_labels": ["fresh", "aging", "stale", "last_known_good", "unknown", "temporarily_unavailable"]}
...
def connector_cache_status() -> Dict[str, Any]:
    ...
    return {"ok": True, "generated_at": _now(), "version_scope": "v1.7.0", "title": "Sustainable Development Connector Cache", "connectors": rows, "counts": {state: sum(1 for row in rows if row["state"] == state) for state in ("fresh", "stale_servable", "expired", "empty")}}
