...
from .authoritative_connectors_v43514 import connector_readiness
...
VERSION=APP_VERSION; CONTRACT='authoritative-api-coverage-production-audit-v43514'
...
def closure_ledger(settings:Any=None):
    ...
    return {'ok':True,'version':VERSION,'contract':CONTRACT,'network_calls_performed':False,'summary':{'machine_readable_gap_records':len(gaps),'registered_not_retrieved':c['REGISTERED'],'configuration_required':c['AUTH_REQUIRED'],'bulk_only':c['BULK'],'stale':c['STALE'],'agriculture_registered_backlog':lookup['Agriculture, Crops & Food Systems']['registered_backlog'],'humanitarian_registered_backlog':lookup['Humanitarian Intelligence']['registered_backlog']},'workspace_ledger':matrix,'gap_records':gaps,'generated_at':_now()}
def production_audit(settings=None):
    ...
    checks={'source_inventory_reconciles':sum(c.values())==len(rows),'connector_catalog_ready':connector_readiness(settings)['ok'],'metric_semantics_and_precedence_ready':evidence_readiness()['ok'],'workspace_truth_unification_ready':workspace_evidence_readiness()['ok'],'no_known_stale_implemented_connectors':c['STALE']==0,'agriculture_registered_backlog_closed':cl['workspaces']['agriculture_food_security']['registered_backlog']==0,'humanitarian_registered_backlog_closed':cl['workspaces']['humanitarian']['registered_backlog']==0,'external_source_health_not_used_as_release_blocker':True}
    ready=all(checks.values()); active=c['LIVE']+c['DISCOVERY']; gated=active+c['AUTH_REQUIRED']
    return {'ok':ready,'version':VERSION,'contract':CONTRACT,'network_calls_performed':False,'production_controls_ready':ready,'coverage_closure_complete':c['REGISTERED']==0,'closure_status':'high-priority-workspaces-closure-iv-complete-backlog-open' if ready and c['REGISTERED'] else 'complete' if ready else 'blocked','summary':base['summary'],'machine_readable_summary':{'registrations':len(rows),'counts':{k:int(c.get(k,0)) for k in prior.ACCESS_CLASSES},'implemented_discovery_or_configuration_gated':gated,'registered_not_retrieved':c['REGISTERED']},'coverage':{'live_or_discovery_pct_of_machine_readable':_pct(active,len(rows)),'implemented_discovery_or_configuration_gated_pct_of_machine_readable':_pct(gated,len(rows)),'registered_not_retrieved_pct_of_machine_readable':_pct(c['REGISTERED'],len(rows))},'checks':checks,'closure_iv':cl,'closure_ledger_summary':ledger['summary'],'next_connector_targets':list(base.get('priority_connector_targets') or []),'generated_at':_now()}
