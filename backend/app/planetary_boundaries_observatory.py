...
def boundary_sources(boundary_id: str | None = None) -> Dict[str, Any]:
    ...
    return {"ok": True, "generated_at": _now(), "version_scope": "v1.9.0", "title": "Planetary Boundary Sources", "summary": "Operational data sources and scientific references supporting the observatory.", "boundary_id": boundary_id, "sources": rows, "connectors": [{**row, "label": row["title"], "status": "registered", "source_mode": row["official_status"], "family": "planetary-boundaries", "public_use": "Supports boundary control-variable context.", "safe_display": row["limitations"]} for row in rows], "references": REFERENCES}
