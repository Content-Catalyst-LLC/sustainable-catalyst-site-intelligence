"""Expanded, balanced public-interest ticker feed for Site Intelligence v3.1.2.

The feed combines verified public events, weather/environment observations,
open-research metadata, and periodic development indicators. It deliberately
leaves all WordPress/Astra navigation and breadcrumb styling to the active
site theme.
"""
...
def _balanced_selection(signals: Iterable[dict[str, Any]], limit: int) -> list[dict[str, Any]]:
    ...
    for signal in ordered:
        ...
        if source_counts[source] >= MAX_SIGNALS_PER_SOURCE:
            continue
...
def build_live_intelligence(settings: Settings, *, category: str = "", limit: int = DEFAULT_SIGNAL_LIMIT) -> dict[str, Any]:
    generated_at = _now()
    ...
    selected = _balanced_selection(all_signals, limit)
    feed_state.update({
        ...
        "maximum_signals_per_source": MAX_SIGNALS_PER_SOURCE,
    })
    return {
        "ok": True,
        "version": APP_VERSION,
