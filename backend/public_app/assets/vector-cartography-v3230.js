(function (window, document) {
  ...
  function inferAssetUrl(filename) {
    const script = document.currentScript || Array.from(document.scripts).find(item => /vector-cartography-v3230\.js/.test(item.src));
    if (script?.dataset?.worldUrl && filename.includes("world-boundaries")) return script.dataset.worldUrl;
    if (script?.src) return script.src.replace(/vector-cartography-v3230\.js(?:\?.*)?$/, filename);
  ...
  function loadBoundaries() {
    ...
    const url = inferAssetUrl("world-cartography-v3230.geojson");
  ...
  class SelfHostedMap {
    ...
    _minimumWorldZoom() {
      ...
      const required = Math.max(dimensions.width / TILE_SIZE, dimensions.height / TILE_SIZE);
      return clamp(Math.ceil(Math.log2(Math.max(1, required))), 1, 4);
    }
    ...
    _bindInteraction() {
      ...
      this._canvas.querySelector("[data-map-reset]")?.addEventListener("click", () => this.setView([0, 20], this._minimumWorldZoom()));
      ...
      this._container.addEventListener("keydown", event => {
        ...
        else if (event.key === "Home") { event.preventDefault(); this.setView([0, 20], this._minimumWorldZoom()); }
      });
    ...
    _drawLabels() {
      ...
      this._boundaries.features
        ...
        .slice(0, this._zoom <= 2 ? 45 : this._zoom <= 3 ? 85 : 150)
        .forEach(feature => {
          const props=feature.properties||{}; const point=this.project([props.label_lat,props.label_lng]);
          const label=props.name||props.iso_a3||"";
          if(point.x < -80 || point.x > dimensions.width+80 || point.y < -30 || point.y > dimensions.height+30 || !label) return;
          const rank=Number(props.label_rank||5);
          const width=Math.max(28,label.length*(rank<=1?7.2:rank<=2?6.2:5.2)); const height=rank<=1?16:13;
          ...
          text.setAttribute("class",`scsi-country-label scsi-label-rank-${rank}`); text.setAttribute("text-anchor","middle");
          text.textContent=label; this._labelRoot.appendChild(text);
        });
    }
    _updateScale() {
    ...
    setView(center, zoom) { this._center = this._normalizeCenter(center); if (Number.isFinite(Number(zoom))) this._zoom = clamp(Number(zoom), this._minimumWorldZoom(), 18); this._redraw(); this._emit("move"); this._emit("zoom"); this._emit("moveend"); this._emit("zoomend"); return this; }
    ...
    fitBounds(bounds, options = {}) {
      ...
      if (Number.isFinite(requestedMax)) chosen = Math.min(chosen, requestedMax);
      chosen = Math.max(chosen, this._minimumWorldZoom());
  ...
  class SelfHostedTileLayer {
    ...
    _redraw() {
      ...
      const z = clamp(Math.max(Math.round(this._map.getZoom()), this._map._minimumWorldZoom()), Number(this.options.minZoom || 0), Number(this.options.maxZoom || 19));
      ...
      for (let tx = minX; tx <= maxX; tx += 1) for (let ty = minY; ty <= maxY; ty += 1) {
        if (this._role === "imagery" && (tx < 0 || tx >= count)) continue;
        ...
        img.style.left = `${Math.floor(tx * TILE_SIZE - center.x + dimensions.width / 2)}px`; img.style.top = `${Math.floor(ty * TILE_SIZE - center.y + dimensions.height / 2)}px`;
        img.style.width = `${TILE_SIZE + 1}px`; img.style.height = `${TILE_SIZE + 1}px`;
        const settle = ok => {
    ...
    _settled() {
      ...
      container.dataset.scsiVisibleTiles = String(this._loaded);
      container.dataset.scsiTileErrors = String(this._errors);
      if (this._loaded > 0) {
  ...
  window.SCSIMapReliability = {
    ...
    getMap: function (id) { return managedMaps.get(id)?.map || null; },
    focus: function (id, center, zoom) { const map=managedMaps.get(id)?.map; if(!map)return false; map.flyTo?.(center, zoom); return true; },
  };
