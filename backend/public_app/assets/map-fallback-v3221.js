(function (window, document) {
  ...
  function markManagedMap(map) {
    ...
    if (container) container.classList.add("scsi-map-managed");
    return map;
  }
  ...
  function patchRealLeaflet(L) {
    ...
    L.tileLayer = function reliableTileLayer(url, options) {
      ...
      layer.on("tileerror", function () {
        ...
        if (container) container.classList.add("scsi-map-tile-degraded");
        if (errors === 1) dispatch("scsi:map-degraded", { reason: "tile-error", source });
        ...
        if (isCarto && errors >= 4 && !fallbackActivated && map) {
          ...
          dispatch("scsi:map-fallback", { reason: "carto-unavailable", source, fallback: OSM_TILES });
        } else if (!isCarto && !isOsm && errors === 4) {
          dispatch("scsi:imagery-degraded", { reason: "imagery-tile-error", source });
        }
      });
      ...
      layer.on("load", function () {
        const map = layer._map;
        const container = map && typeof map.getContainer === "function" ? map.getContainer() : null;
        if (container) container.classList.remove("scsi-map-tile-degraded");
      });
      return layer;
    };
  ...
  class StaticMap {
    constructor(target, options = {}) {
      ...
      this._drawGrid();
      dispatch("scsi:map-fallback", { reason: "leaflet-unavailable", mode: "static" });
    }
    _drawGrid() {
