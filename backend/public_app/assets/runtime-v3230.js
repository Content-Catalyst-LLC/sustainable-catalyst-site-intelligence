(function (window, document) {
  ...
  const ENDPOINTS = [
    ...
    ["Release Gate", "/public/release-gate?plugin_version=4.35.3.1&expected_release_id=site-intelligence-v4.35.3.1"],
  ...
  function recordError(type, message, source) {
    const normalizedMessage = text(message).slice(0, 400);
    const normalizedSource = text(source).slice(0, 240);
    const fingerprint = [type, normalizedMessage, normalizedSource].join("|");
    const existing = state.errors.find(function (item) { return item.fingerprint === fingerprint; });
    if (existing) {
      existing.active = true;
      existing.lastSeenAt = new Date().toISOString();
      existing.occurrences = Number(existing.occurrences || 1) + 1;
    } else {
      state.errors.unshift({ type, message: normalizedMessage, source: normalizedSource, fingerprint, active: true, occurrences: 1, at: new Date().toISOString(), lastSeenAt: new Date().toISOString() });
    }
    state.errors = state.errors.slice(0, EVENT_LIMIT);
    updateUi();
  }

  function resolveErrors(reason) {
    const resolvedAt = new Date().toISOString();
    state.errors.forEach(function (item) {
      if (item.active) { item.active = false; item.resolvedAt = resolvedAt; item.resolution = reason || "runtime-recovered"; }
    });
  }
  ...
  function mapSnapshot() {
    ...
    return {
      ...
      libraryMode: window.L?.__scsiSelfHosted ? "vector-cartography-engine" : window.L?.__scsiFirstParty ? "first-party-interactive" : window.L?.__scsiFallback ? "geographic-fallback" : window.L ? "leaflet" : "unavailable",
      mapCount: containers.length,
      modes: containers.reduce(function (counts, container) {
        const mode = container.dataset.scsiMapMode || (container.classList.contains("scsi-self-hosted-map") ? "self-hosted-local-vector" : "declared");
  ...
  function visibleMapContainers() {
    return Array.from(document.querySelectorAll(".scsi-map-managed, .scsi-static-map, [id$='Map']")).filter(function (element) {
      ...
      return {
        ...
        mode: element.dataset.scsiMapMode || (element.classList.contains("scsi-self-hosted-map") ? "self-hosted-local-vector" : "map-pending"),
        degraded: element.dataset.scsiMapStatus === "failed",
        imageryMode: element.dataset.scsiImageryMode || "normal",
      };
    });
  }
  ...
  function reportObject() {
    return {
      ...
      activeErrorCount: state.errors.filter(function (item) { return item.active; }).length,
  ...
  function mapRows() {
    ...
    return '<div class="scsi-runtime-metric-grid">' +
      ...
      '<div><strong>' + escapeHtml(snapshot.imageryLimitedCount || 0) + '</strong><span>Imagery limited</span></div>' +
  ...
  function eventRows() {
    ...
    return '<div class="scsi-runtime-list">' + errors.map(function (item) {
      return '<div class="scsi-runtime-row"><span class="scsi-runtime-state is-' + (item.active ? 'fail' : 'pass') + '">' + (item.active ? 'Error' : 'Resolved') + '</span><strong>' + escapeHtml(item.type) + '</strong><small>' + escapeHtml(item.message) + '</small></div>';
  ...
  function updateUi() {
    ...
    if (body) body.innerHTML = '<section><h3>Application contract</h3>' + endpointRows() + '</section><section><h3>Service recovery</h3>' + serviceRows() + '</section><section><h3>Map-by-map health</h3>' + mapRows() + '</section><section><h3>Session faults</h3>' + eventRows() + '</section><p class="scsi-runtime-footnote">Diagnostics are local and public-safe. Hidden workspaces and optional imagery outages do not lower application health. The self-hosted map engine remains healthy on its local world-boundary basemap. Recovered browser errors are retained as resolved history, not active failures.</p>';
  }
  ...
  async function runChecks() {
    ...
    const maps = mapSnapshot();
    const visibleMaps = (maps.surfaces || visibleMapContainers()).filter(function (surface) { return surface.visible !== false; });
    const criticalHealthy = results.every(function (item) { return item.ok; }) && !visibleMaps.some(function (surface) { return surface.degraded || surface.status === "degraded"; }) && state.contract?.status !== "degraded";
    if (criticalHealthy) resolveErrors("successful-runtime-check");
    state.running = false;
    updateUi();
  ...
  ["scsi:map-library-ready", "scsi:map-first-party-ready", "scsi:map-initialized", "scsi:map-degraded", "scsi:map-fallback", "scsi:map-recovered", "scsi:map-retry", "scsi:imagery-degraded", "scsi:service-recovery-ready", "scsi:service-retry", "scsi:service-fallback", "scsi:service-circuit-open", "scsi:service-recovered"].forEach(function (name) {
    window.addEventListener(name, function (event) {
      recordEvent(name, event.detail || {});
      if (name === "scsi:map-recovered" || name === "scsi:service-recovered" || name === "scsi:map-local-basemap-ready") resolveErrors(name);
    });
  });
