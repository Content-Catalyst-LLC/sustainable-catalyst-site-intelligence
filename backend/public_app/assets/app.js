...
(() => {
  ...
  async function loadCountry(code){
    ...
    // v4.36.1: focus immediately from the first-party canonical registry.
  ...
  async function loadCountryCatalog(){
    ...
    globalCountryState.catalogPromise=(async()=>{
      ...
      // v4.36.1: live provider metadata may enrich a country, but it must never
  ...
  async function selectGlobalCountry(code,pushState=true){
    ...
    // v4.36.1: commit the selected identity to the URL before any upstream work.
  ...
  // registered route recovery is enforced after every route transition by v4.36.1.
  ...
  window.SCSIRouterV3228={version:"4.36.1",navigate:navigateToRoute,current:()=>state.route};
...
/* v4.36.1 publishing integration: window.SCIntelligencePublishingV2200 */
/* v4.36.1 scheduled monitoring integration: window.SCScheduledMonitoringV2210 */
