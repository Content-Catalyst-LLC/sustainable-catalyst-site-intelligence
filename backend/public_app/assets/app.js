...
(() => {
  ...
  const RELEASE_LINEAGE="v4.38.0";
  const SCIENCE_CONTROLLER_SRC="/app/assets/science-v240.js?v=4.38.0";
  let scienceControllerPromise=null;
  function scienceControllerCurrent(){
    try{return window.SCScienceV240?.open&&window.SCScienceV240?.status?.().repair==="4.38.0"?window.SCScienceV240:null}catch{return null}
  }
  async function ensureScienceController(){
    const current=scienceControllerCurrent();if(current)return current;
    if(scienceControllerPromise)return scienceControllerPromise;
    scienceControllerPromise=new Promise((resolve,reject)=>{
      document.querySelectorAll('script[src*="/science-v240.js"]').forEach(node=>node.remove());
      const script=document.createElement("script");script.src=new URL(SCIENCE_CONTROLLER_SRC,window.SC_SITE_INTELLIGENCE_API||location.origin).href;script.async=true;script.dataset.scsiRepairLoader="science-r4";
      script.onload=()=>{const controller=scienceControllerCurrent();controller?resolve(controller):reject(new Error("Science controller loaded without the R4 runtime contract."))};
      script.onerror=()=>reject(new Error("Science controller could not be loaded."));
      document.head.appendChild(script);
    }).finally(()=>{scienceControllerPromise=null});
    return scienceControllerPromise;
  }
  function revealScienceRecovery(message){
  ...
  async function openFeaturedScienceDomain(domain="space"){
    ...
    const opened=await Promise.resolve(controller.openDomain?.(domain));
    if(opened===false)throw new Error(`Science domain ${domain} could not be opened.`);
    if(domain==="space"&&window.SCSILiveSpaceV4380?.enter){
      const liveOpened=await Promise.resolve(window.SCSILiveSpaceV4380.enter());
      if(liveOpened===false)throw new Error("Live Space observation workspace could not be opened.");
    }
    return true;
  }
  ...
  async function loadCountry(code){
    ...
    // v4.38.0: focus immediately from the first-party canonical registry.
  ...
  async function loadCountryCatalog(){
    ...
    globalCountryState.catalogPromise=(async()=>{
      ...
      // v4.38.0: live provider metadata may enrich a country, but it must never
  ...
  async function selectGlobalCountry(code,pushState=true){
    ...
    // v4.38.0: commit the selected identity to the URL before any upstream work.
  ...
  function renderCredentialConfiguration(data){
    const states=data?.states||{};
    const configured=states.configured??0, profiles=data?.profile_count??0;
    const unresolved=(states.missing??0)+(states.partial??0)+(states.invalid??0);
    qs("#credentialProfilesMetric").textContent=profiles||"—";
    qs("#credentialConfiguredMetric").textContent=configured;
    qs("#credentialMissingMetric").textContent=unresolved;
    qs("#credentialMappedMetric").textContent=data?.mapped_auth_required_registrations??"—";
    qs("#credentialConfigurationStatus").textContent=data?.configuration_complete===true?`Credential configuration complete · ${configured}/${profiles} profiles configured.`:`Credential control plane ready · ${configured}/${profiles} profiles configured · ${unresolved} need configuration. Missing credentials do not block deployment.`;
  ...
  function renderProductionSoak(data){
    const flap=(data?.scenarios||[]).find(item=>item.id==="24-cycle-provider-flapping");
    qs("#productionSoakScenarioMetric").textContent=data?.scenario_count!=null?`${data.passed_scenario_count||0} / ${data.scenario_count}`:"—";
    qs("#productionSoakFlapMetric").textContent=flap?.cycles??24;
    qs("#productionSoakSemanticMetric").textContent=data?.checks?.semantic_truth_guard_ready===true?"Strict":"Review";
    qs("#productionSoakBlockingMetric").textContent=data?.upstream_health_release_blocking===false?"No":"Review";
    qs("#productionSoakStatus").textContent=data?.ok===true?"Production soak ready · 8 deterministic failure/recovery scenarios pass · real-provider health remains non-blocking.":"Production soak requires review.";
  }
  ...
  function renderEvidenceIntelligence(data){
    ...
    qs("#evidenceFreshnessMetric").textContent=data?.freshness_cadence_count??"—";
    qs("#evidenceIntelligenceStatus").textContent=data?.critical_boundary||"Metric semantics, source precedence, and freshness policy are available.";
  }
  ...
  async function loadSourceStudio(pushState=false){
    ...
    try{
      const [sourcesResult,methodsResult,auditResult,evidenceResult,credentialResult,resilienceResult,soakResult,workspaceAuditResult]=await Promise.allSettled([
        ...
        apiWithRetry("/public/production-soak",1,{signal:controller.signal,timeout:12000}),
  ...
  const savedViewDefinitions={
    ...
    earth:{label:"Earth / Ocean Observation",keys:["earthLayer","dateA","dateB","opacity","swipe","orbitMode","orbitAlt","oceanMode","oceanSystem","mapLat","mapLng","mapZoom"]},
  ...
  function currentSavedState(){
    ...
    if(route==="earth"){const p=new URLSearchParams(location.search);values={earthLayer:qs("#earthLayerSelect").value,dateA:qs("#earthDateA").value,dateB:qs("#earthDateB").value,opacity:qs("#earthOpacity").value,swipe:qs("#earthSwipe").value,oceanMode:p.get("oceanMode")||"",oceanSystem:p.get("oceanSystem")||""};}
  ...
  // registered route recovery is enforced after every route transition by v4.38.0.
  async function setRoute(route){
    ...
    if(route==="science"){
      ...
      try{
        ...
        revealScienceRecovery("Science discovery opened in recovery mode. Earth, Ocean, and Space remain locally available; retry if controls are not active.");
      }
      window.SCSIProductionTruthV3231?.evaluate?.();return;
    }
  ...
  async function init(){setLaunch("Preparing the map and public evidence services.",18);
    ...
    qsa('[data-space-entry="hub"]').forEach(button=>button.addEventListener("click",()=>openFeaturedScienceDomain("space").catch(error=>{console.error("[Site Intelligence] Space featured entry failed",error);revealScienceRecovery("Space Intelligence opened in Science recovery mode. Retry if the six local space workspaces are not visible.")})));
...
/* v4.38.0 publishing integration: window.SCIntelligencePublishingV2200 */
/* v4.38.0 scheduled monitoring integration: window.SCScheduledMonitoringV2210 */
