...
if [[ "${SC_SI_SKIP_TESTS:-0}" != "1" ]]; then
  ...
  [[ "$COLLECTED" == "1697" ]] || { echo "ERROR: expected 1697 collected tests, got ${COLLECTED:-unknown}" >&2; exit 1; }
  TEST_FILES=("$BACKEND"/tests/test_*.py); CHUNK_SIZE="${SC_SI_PYTEST_CHUNK_FILES:-57}"; OFFSET=0; CHUNK=1; PASSED=0
...
if [[ "${SC_SI_RUN_BROWSER:-0}" == "1" ]]; then
  for script in browser_underwater_media_v4370.py browser_ocean_observation_v4360.py browser_science_ocean_workspace_controller_v4360_r4.py browser_palestine_navigation_integrity_v4360.py browser_country_evidence_presentation_v4360.py; do
...
echo 'SUCCESS: Site Intelligence v4.39.1 passed live underwater media discovery, three-provider readiness, inherited 11-system Ocean, Ocean/Space prominence, country/evidence, and 35-route platform validation.'
