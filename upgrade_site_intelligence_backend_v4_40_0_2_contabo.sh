...
ARCHIVE="${1:-/tmp/sustainable-catalyst-site-intelligence-backend-v4.40.0.2.zip}"
...
TMP="$(mktemp -d /tmp/sc-site-v44002.XXXXXX)"
trap 'rm -rf "$TMP"' EXIT
...
grep -q "RELEASE_NAME = \"Homepage Intelligence Contract & Ticker Recovery\"" "$version_file" || fail "release name mismatch"
...
print('PASS: v4.40.0.2 capability contract and ticker feed route are aligned')
print(metrics)
PYVERIFY
...
python3 - "$health" <<'PY'
import json,sys
x=json.loads(sys.argv[1]); assert x.get('ok') is True; assert x.get('version')=='4.40.0.2', x
print('PASS: local health reports Site Intelligence 4.40.0.2')
PY
...
gate="$(curl -fsS "http://127.0.0.1:${PORT}/public/release-gate?plugin_version=4.40.0.2&expected_release_id=site-intelligence-v4.40.0.2")"
python3 - "$gate" <<'PY'
import json,sys
x=json.loads(sys.argv[1]); assert x.get('ok') is True; assert x.get('install_allowed') is True; assert x.get('version')=='4.40.0.2', x
print('PASS: local release gate allows WordPress 4.40.0.2')
PY
...
import json,sys
x=json.loads(sys.argv[1]); raise SystemExit(0 if x.get('version')=='4.40.0.2' else 1)
PY
    then break; fi
    sleep 3
  done
...
import json,sys
x=json.loads(sys.argv[1]); assert x.get('ok') is True; assert x.get('version')=='4.40.0.2', x
print('PASS: public backend health reports 4.40.0.2')
PY
...
import json,sys
x=json.loads(sys.argv[1]); assert x.get('version')=='4.40.0.2', x
m={str(v.get('id')):v.get('value') for v in x.get('metrics',[]) if isinstance(v,dict)}
expected={'country_profiles':172,'enabled_connectors':14,'public_workspaces':35,'live_feeds':8}
assert m == expected, (m,expected)
assert isinstance(x.get('featured_signal_count'),int) and x['featured_signal_count'] >= 0
...
echo "Backup: $BACKUP"
echo "NEXT: install the v$VERSION WordPress plugin, clear caches, then verify 172 / 14 / 35 / 8 plus the integrated scrolling ticker in the homepage widget."
