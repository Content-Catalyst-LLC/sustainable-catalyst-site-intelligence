...
for ((attempt=1; attempt<=ATTEMPTS; attempt++)); do
  ...
  if [[ -n "$payload" ]]; then
    ...
    if [[ "$OBSERVED_VERSION" == "$RELEASE" && "$OBSERVED_RELEASE_ID" == "$RELEASE_ID" && "$OBSERVED_GATE" == "ready" && ( "$OBSERVED_COMMIT" == "$COMMIT" || "$COMMIT" == "$OBSERVED_COMMIT"* || "$OBSERVED_COMMIT" == "$COMMIT"* ) ]]; then
      ...
      [[ "$app_html" == *'data-scsi-release="4.10.0"'* && "$app_html" == *'vector-cartography-v3230.js'* && "$app_html" == *'cartographic-workspace-v3230.js'* && "$app_html" == *'cartographic-interaction-v3232.js'* ]] && app_ready=true
...
' "$OBSERVED_VERSION" "${OBSERVED_COMMIT:0:12}"
        printf '
SUCCESS: Site Intelligence v4.10.0 is live with record provenance and indicator truth.
