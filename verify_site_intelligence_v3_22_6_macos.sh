...
RUNTIME_SANDBOX="$(mktemp -d "${TMPDIR:-/tmp}/scsi-v3226-runtime.XXXXXX")"
...
grep -q 'DEPLOYMENT_RECEIPT=' "$ROOT/promote_site_intelligence_v3_22_6_to_github_and_render_macos.sh"
printf '
...
assert manifest['release']=='3.22.6'; assert manifest['file_count']==len(manifest['files'])
...
printf '
SUCCESS: Site Intelligence v3.22.6 passed deterministic local validation.
Repository: %s
