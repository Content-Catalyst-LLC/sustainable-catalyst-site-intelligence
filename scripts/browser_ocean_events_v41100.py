...
def fixture_html():
    catalog={
      'ok':True,'version':'4.15.0','contract':'ocean-events-hazards-ecosystem-change',
      'sources':[
...
window.fetch=async input=>{const u=String(input);let x=catalog;if(u.includes('/state')){const qp=new URL(u).searchParams,source=qp.get('source')||'noaa-coral-reef-watch',hazard=qp.get('hazard_type')||'marine-heatwave',s=catalog.sources.find(r=>r.id===source)||catalog.sources[0],h=catalog.hazard_types.find(r=>r.id===hazard)||catalog.hazard_types[0];x={ok:true,version:'4.15.0',contract:'ocean-events-hazards-ecosystem-change',source:s,hazard_type:h,query_point:qp.has('latitude')?{latitude:Number(qp.get('latitude')),longitude:Number(qp.get('longitude'))}:null,date:qp.get('date')||null,source_supports_hazard_type:(s.hazard_types||[]).includes(h.id),evidence:{condition_record_loaded:false,source_event_loaded:false,threshold_evaluated:false,official_advisory_loaded:false,ecosystem_impact_observed:false},truth:{hazard_declared:false,warning_issued_by_platform:false,forecast_treated_as_observation:false,model_treated_as_in_situ:false,threshold_treated_as_event:false,zero_records_treated_as_safe:false,source_advisory_reissued_by_platform:false}};}return new Response(JSON.stringify(x),{status:200,headers:{'Content-Type':'application/json'}})};
...
def exercise(page,label):
    ...
    page.wait_for_function("window.SCSIOceanEventsV41100?.version==='4.15.0'")
    ...
    assert m['version']=='4.15.0' and m['source']=='noaa-nccos' and m['hazard']=='harmful-algal-bloom'
...
def main():
    ...
    print(json.dumps({'browser':path,'results':[r1,r2]},indent=2)); print('PASS: v4.15.0 Ocean Events, Hazards & Ecosystem Change passed direct and iframe interaction.'); os._exit(0)
