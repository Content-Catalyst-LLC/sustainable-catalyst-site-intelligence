...
req('APP_VERSION = "4.38.0"' in version,'backend APP_VERSION is not 4.38.0')
req('Version: 4.38.0' in plugin and 'site-intelligence-v4.38.0' in plugin,'WordPress release identity mismatch')
...
req('const VERSION="4.38.0"' in space_js,'live Space browser controller not 4.38.0')
...
for token in ('data-ocean-entry="hub"','data-space-entry="hub"','liveSpaceObservation','/app/assets/live-space-observation-v4380.js?v=4.38.0','/app/assets/iframe-navigation-v4380.css?v=4.38.0','/app/assets/app.js?v=4.38.0'):
    req(token in html,f'public app missing {token}')
req(html==html_wp,'WordPress index mirror differs from backend')
req('const RELEASE="4.38.0"' in sw and 'const REPAIR="v4380"' in sw,'service worker is not on v4.38 cache lineage')
...
if errors:
    ...
    raise SystemExit(1)
print('PASS: v4.38.0 Live Space Observation + iframe navigation release contract')
