...
c=TestClient(app)
p=c.get('/public/ocean-missions').json(); assert p['ok'] and p['version']=='4.12.0' and p['contract']=='ocean-missions-vehicles-observatory-network' and p['route']=='earth' and p['source_count']==4 and p['platform_type_count']>=10
...
bio=c.get('/public/marine-biodiversity/readiness').json(); assert bio['ok'] and bio['version']=='4.12.0'
nav=c.get('/public/v4/navigation').json(); assert nav['route_count']==35 and nav['primary_area_count']==6
print('PASS: v4.12.0 ocean missions / vehicles / observatory release contract')
