...
def exercise(page, label: str) -> dict[str, object]:
    ...
    page.evaluate(
        """()=>{
          ...
          for(let i=0;i<12;i++){
            window.dispatchEvent(new CustomEvent('scsi:workspace-state',{detail:{version:'3.28.0',route:'overview',state:i%2?'ready':'degraded'}}));
...
def main() -> int:
    ...
    print("PASS: v3.28.0 preserves country-picker focus, leaves wheel interaction unblocked, and selects the first, last, and representative catalog countries in direct and iframe modes.")
    return 0
