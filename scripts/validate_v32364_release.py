...
required={
 ...
 'wordpress-plugin/sustainable-catalyst-site-intelligence/sustainable-catalyst-site-intelligence.php':['Version: 4.10.0','loading="eager"','fetchpriority="high"'],
...
if not contract.get('ok') or contract.get('version')!='4.10.0':raise SystemExit('Startup-stability endpoint failed.')
...
if manifest.is_file() and json.loads(manifest.read_text()).get('release')!='4.10.0':raise SystemExit('Manifest release mismatch.')
print('Site Intelligence v4.10.0 production-soak, route-stability, and service-worker closure contract passed.')
