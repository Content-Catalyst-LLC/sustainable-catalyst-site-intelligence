...
c=TestClient(app)
p=c.get('/public/seafloor-intelligence').json(); assert p['ok'] and p['version']=='4.16.0' and p['contract']=='seafloor-bathymetric-intelligence' and p['route']=='earth'
...
water=c.get('/public/water-column/readiness').json(); assert water['ok'] and water['version']=='4.16.0'
nav=c.get('/public/v4/navigation').json(); assert nav['route_count']==35 and nav['primary_area_count']==6
print('PASS: v4.16.0 seafloor/bathymetry release contract')
