#!/usr/bin/env python3
"""Network-independent Chromium smoke test for v3.23.6.3 map interaction controls."""
...
def main() -> int:
    ...
    with sync_playwright() as playwright:
        ...
        page.set_content('<!doctype html><html><body><div id="app" data-scsi-release="3.23.6.3" class="app-ready"><section class="map-panel"><div class="map-toolbar"><div class="map-actions"></div></div><div id="map" style="width:1000px;height:560px"></div><div id="eventList"></div></section></div></body></html>')
        ...
        page.evaluate("features => { history.replaceState=()=>{}; const map=L.map('map',{minZoom:2,maxZoom:9}).setView([0,38],3); let filters={categories:[],source:'',days:30,cluster:true,eventsVisible:true,selected:''}; let filtered=features.slice(); const redraw=()=>{filtered=features.filter(f=>(!filters.categories.length||filters.categories.includes(f.properties.category))&&(!filters.source||filters.source===f.properties.source)); window.dispatchEvent(new CustomEvent('scsi:overview-events-rendered',{detail:{count:filtered.length}}));}; window.SCSIOverviewMapV3232={version:'3.23.6.3',getMap:()=>map,getEvents:()=>features,getFilteredEvents:()=>filtered,getFilters:()=>({...filters}),setFilters:next=>{filters={...filters,...next};redraw();return filters},selectEvent:id=>{filters.selected=id},fitResults:()=>map.fitBounds(filtered.map(f=>[f.geometry.coordinates[1],f.geometry.coordinates[0]]),{maxZoom:6}),setImageryOpacity:value=>{document.querySelector('#map').dataset.opacity=String(value)},setBaseStyle:value=>{document.querySelector('#map').dataset.mapStyle=value},syncUrl:()=>{},render:redraw}; redraw(); }", features)
    ...
    print("PASS: v3.23.6.3 rendered layer controls, semantic filters, synchronized result state, fit-to-results, and local geography in Chromium.")
    return 0
