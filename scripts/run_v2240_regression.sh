...
RUNTIME_DIR="${2:-$(mktemp -d "${TMPDIR:-/tmp}/scsi-v2240-regression.XXXXXX")}" 
...
mkdir -p "$RUNTIME_DIR"/{country,history,spatial,harmonization,models,evidence,graph,publishing,monitoring,workspaces,workflows,federation,connectors,platform}
...
export SC_SI_FEDERATION_INSTITUTIONS_PATH="$RUNTIME_DIR/federation/institutions_v2240.jsonl"
export SC_SI_FEDERATION_RECORDS_PATH="$RUNTIME_DIR/federation/records_v2240.jsonl"
export SC_SI_FEDERATION_MANIFESTS_PATH="$RUNTIME_DIR/federation/manifests_v2240.jsonl"
export SC_SI_FEDERATION_IMPORTS_PATH="$RUNTIME_DIR/federation/imports_v2240.jsonl"
export SC_SI_FEDERATION_TRUST_PATH="$RUNTIME_DIR/federation/trust_v2240.jsonl"
