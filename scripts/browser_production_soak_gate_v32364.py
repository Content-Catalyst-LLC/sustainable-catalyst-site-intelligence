...
def main():
  ...
  for mode in ('disabled','failure'):
   ...
   for _ in range(3):
    for route in ('science','overview','country','overview'):
     page.evaluate('(route)=>window.SCSIRouterV3228.navigate(route)',route);page.wait_for_timeout(80)
 ...
 print(json.dumps({'browser':executable,'results':results},indent=2));print('PASS: v4.17.0 opened its shell without network dependency, serialized route churn, and performed no service-worker reload.');return 0
