...
def fixture_html():
    catalog={
      'ok':True,'version':'4.14.0','contract':'underwater-observation-visual-evidence',
      'sources':[
...
def exercise(page,label):
    ...
    page.wait_for_function("window.SCSIUnderwaterV4800?.version==='4.14.0'")
    ...
    assert m['version']=='4.14.0' and m['source']=='fathomnet' and m['media']=='still-image'
...
def main():
    ...
    print(json.dumps({'browser':path,'results':[r1,r2]},indent=2)); print('PASS: v4.14.0 Underwater Observation & Visual Evidence passed direct and iframe interaction.'); sys.stdout.flush(); os._exit(0)
