#!/usr/bin/env python3
"""Overview-route browser regression for the v4.35.24 Palestine navigation repair."""
...
def main() -> int:
    ...
    if not browser_path:
        print("ERROR: Chromium or Chrome is required for v4.35.24 Palestine navigation gate.")
        return 2
    try:
        from playwright.sync_api import sync_playwright
    except ImportError:
        print("ERROR: Playwright is required for v4.35.24 Palestine navigation gate.")
        return 2
...
(()=>{
  ...
  window.fetch=async(input,init)=>{
    ...
    if(path==='/public/countries'){
      ...
      window.__v43524FetchLog.push('hostile-catalog');
    ...
    if(m){
      ...
      window.__v43524FetchLog.push(`${requested}:country-intelligence-as-${wrong}`);
      return new Response(JSON.stringify({ok:true,version:'4.35.24',country_code:wrong,registered_source_count:1,domain_summaries:[]}),{status:200,headers:{'Content-Type':'application/json'}});
    }
    m=path.match(/^\/public\/country\/(ISR|PSE)\/overview$/);
    if(m){
      const requested=m[1], wrong=requested==='ISR'?'PSE':'ISR', h=hostileCountries[requested];
      window.__v43524FetchLog.push(`${requested}:overview-as-${wrong}`);
      return new Response(JSON.stringify({ok:true,version:'4.35.24',country:{code:wrong,iso2:h.iso2,name:h.name,latitude:h.latitude,longitude:h.longitude},map:{default_zoom:8},highlights:[]}),{status:200,headers:{'Content-Type':'application/json'}});
    }
...
"""
    ...
    with sync_playwright() as pw:
        ...
        results["log"]=page.evaluate("window.__v43524FetchLog")
        browser.close()
    ...
    print("PASS: v4.35.24 keeps Palestine and Israel canonical on the Overview selector even when external catalog/overview payloads attempt to swap them.")
    return 0
