...
with sync_playwright() as p:
 ...
 page.set_content('<!doctype html><style>'+CSS+'</style><div id="app" data-scsi-release="3.23.7"><header><div class="topbar-controls"><select id="countrySelect"><option value="BRA">Brazil</option><option value="KEN">Kenya</option></select></div></header></div>')
 ...
 page.add_script_tag(content=JS);page.click('#dataTruthToggle');page.wait_for_selector('text=Brazil (BRA)');country_ready=page.locator('text=Brazil (BRA)').count()>0;page.click('[data-truth-view="matrix"]');page.wait_for_selector('.scsi-coverage-matrix [data-country-search*="kenya"]');page.fill('#dataTruthMatrixFilter','Brazil');hidden=page.locator('[data-country-search]').nth(1).is_hidden();result={"errors":errors,"country_ready":country_ready,"matrix_ready":page.locator('.scsi-coverage-matrix').count()==1,"filter_hides":hidden,"api":page.evaluate('window.SCSIDataTruthV3237.version')};b.close()
print(json.dumps(result,indent=2));assert not errors and result['country_ready'] and result['matrix_ready'] and result['filter_hides'] and result['api']=='3.23.7';print('PASS: v3.23.7 country truth and coverage matrix rendered without browser errors.')
