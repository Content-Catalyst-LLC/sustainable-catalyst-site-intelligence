...
c=TestClient(app)
s=c.get('/public/institutional-governance');assert s.status_code==200;s=s.json();assert s['version']=='4.16.0' and s['contract']=='institutional-workspaces-review-governance' and s['public_accounts_required'] is False and s['automatic_evidence_approval'] is False
...
html=(BACKEND/'public_app/index.html').read_text();worker=(BACKEND/'public_app/service-worker.js').read_text();js=(BACKEND/'public_app/assets/institutional-governance-v3300.js').read_text();assert 'institutional-governance-v3300.js?v=4.16.0' in html and 'institutional-governance-v3300.js' in worker and 'SCSIInstitutionalGovernanceV3300' in js
print(json.dumps({'version':s['version'],'workspace_sha256':w['workspace_sha256'],'review_queue':q['count'],'import_compatible':imp['compatible']},indent=2));print('PASS: Site Intelligence v4.16.0 institutional workspace and review governance contracts are complete.')
