#!/usr/bin/env python3
"""Network-independent Chromium validation for v3.23.6.4 bootstrap ownership and loading recovery."""
...
def shell(deadline=600):
    return f'''<!doctype html><html><body><div id="launchScreen"><p id="launchMessage">Opening</p><span id="launchProgressBar"></span><button id="launchRetry" hidden>Retry</button></div><div id="app" class="app-loading" data-scsi-release="3.23.6.4" data-scsi-startup-deadline-ms="{deadline}"></div></body></html>'''
...
def main():
    ...
    with sync_playwright() as pw:
        ...
        page=browser.new_page();page.on('pageerror',lambda e:errors.append(f'no-worker:{e}'));page.set_content(shell());page.evaluate(fake_worker());page.add_script_tag(content=JS);page.evaluate("setTimeout(()=>window.dispatchEvent(new CustomEvent('scsi:application-ready',{detail:{version:'3.23.6.4',state:'ready'}})),30)");page.wait_for_timeout(120);results['no_existing_worker']=snapshot(page);page.close()
        # Older/current controller with waiting update: update can activate but rendering remains independent.
        page=browser.new_page();page.on('pageerror',lambda e:errors.append(f'older-worker:{e}'));page.set_content(shell());page.evaluate(fake_worker(controller=True,waiting=True));page.add_script_tag(content=JS);page.evaluate("window.dispatchEvent(new CustomEvent('scsi:application-ready',{detail:{version:'3.23.6.4',state:'ready'}}))");page.wait_for_timeout(80);results['older_worker']=snapshot(page)|{'activation':page.evaluate('window.__activationMessage?.type||""')};page.close()
    ...
    print(json.dumps(results,indent=2));print('PASS: v3.23.6.4 single-owner bootstrap and bounded loading recovery passed all startup conditions.');return 0
