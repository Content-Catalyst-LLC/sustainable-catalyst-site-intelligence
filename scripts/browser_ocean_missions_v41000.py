...
def fixture_html():
    catalog={
      'ok':True,'version':'4.10.0','contract':'ocean-missions-vehicles-observatory-network',
      'sources':[
...
def exercise(page,label):
    ...
    page.wait_for_function("window.SCSIOceanMissionsV41000?.version==='4.10.0'")
    ...
    assert m['version']=='4.10.0' and m['source']=='argo' and m['type']=='float'
...
def main():
    ...
    print(json.dumps({'browser':path,'results':[r1,r2]},indent=2)); print('PASS: v4.10.0 Ocean Missions, Vehicles & Observatory Network passed direct and iframe interaction.'); os._exit(0)
