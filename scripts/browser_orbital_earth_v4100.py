...
def page_html():
    payload={
      'ok':True,'version':'4.6.0','contract':'orbital-earth-satellite-observation','mode':'orbital','presentation':'2.5d-orbital-perspective',
    ...
    export={'ok':True,'version':'4.6.0','schema':'sc-site-intelligence-orbital-view/1.0','orbital_state':payload,'review':{'real_satellite_imagery':True,'live_spacecraft_position_claimed':False,'instantaneous_swath_claimed':False,'human_interpretation_required':True},'manifest_sha256':'a'*64}
    setup=f'''<script>
...
def exercise(page,label):
    page.wait_for_function("window.SCSIOrbitalEarthV4100?.version==='4.6.0'",timeout=5000)
...
def main():
    ...
    for r in results:
        m=r['metrics'];assert m['active'] and not m['hidden'];assert m['altitude']=='35786';assert '35,786' in m['label'];assert 'VIIRS' in m['platform'];assert 'no position fabricated' in m['truth'].lower();assert 'pass-specific sensor footprint' in m['truth'].lower();assert m['map'] and m['version']=='4.6.0' and m['surfaceRestored']
    print(json.dumps({'browser':path,'results':results,'errors':errors},indent=2));print('PASS: v4.6.0 Orbital Earth interaction passed in direct and iframe modes.')
