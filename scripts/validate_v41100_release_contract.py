...
c=TestClient(app)
p=c.get('/public/ocean-events').json(); assert p['ok'] and p['version']=='4.12.0' and p['contract']=='ocean-events-hazards-ecosystem-change' and p['route']=='earth' and p['source_count']==4 and p['hazard_type_count']>=8
...
missions=c.get('/public/ocean-missions/readiness').json(); assert missions['ok'] and missions['version']=='4.12.0'
nav=c.get('/public/v4/navigation').json(); assert nav['route_count']==35 and nav['primary_area_count']==6
print('PASS: v4.12.0 ocean events / hazards / ecosystem change release contract')
