...
c=TestClient(app)
s=c.get('/public/publication-studio'); assert s.status_code==200; s=s.json(); assert s['version']=='4.15.0' and s['contract']=='briefing-story-map-publication-studio' and s['human_editorial_review_required'] is True and s['automatic_publication'] is False and s['public_write_performed'] is False
...
html=(BACKEND/'public_app/index.html').read_text(); worker=(BACKEND/'public_app/service-worker.js').read_text(); js=(BACKEND/'public_app/assets/briefing-publication-v3290.js').read_text(); assert 'briefing-publication-v3290.js?v=4.15.0' in html and 'briefing-publication-v3290.js' in worker and 'SCSIBriefingPublicationV3290' in js
policy=json.loads((BACKEND/'data/briefing_publication_policy_v3290.json').read_text()); assert policy['version']=='4.15.0' and 'retraction' in policy['correction_actions']
print(json.dumps({'version':s['version'],'manifest_sha256':manifest['manifest_sha256'],'readiness':ready['status'],'story_accessibility':story['accessibility']['status'],'correction_preserves_prior_version':correction['preserves_prior_version']},indent=2)); print('PASS: Site Intelligence v4.15.0 Briefing, Story Map, and Publication Studio contracts are complete.')
