...
c=TestClient(app)
p=c.get('/public/marine-biodiversity').json(); assert p['ok'] and p['version']=='4.10.0' and p['contract']=='marine-biodiversity-bioacoustic-intelligence' and p['route']=='earth' and p['source_count']==4 and p['evidence_class_count']==6
...
uw=c.get('/public/underwater-observation/readiness').json(); assert uw['ok'] and uw['version']=='4.10.0'
nav=c.get('/public/v4/navigation').json(); assert nav['route_count']==35 and nav['primary_area_count']==6
print('PASS: v4.10.0 marine biodiversity / bioacoustic release contract')
