...
assert a['machine_readable_summary']['registered_not_retrieved']==37
assert a['machine_readable_summary']['implemented_discovery_or_configuration_gated']==64
assert a['machine_readable_summary']['counts']['STALE']==0
assert production_readiness(settings)['ok'] is True
assert closure_ledger(settings)['summary']['registered_not_retrieved']==37
c=connector_catalog(settings)
assert c['connector_count']==32 and c['live_connector_count']==23
assert c['discovery_connector_count']==4 and c['auth_required_connector_count']==5
...
for marker in ('authoritative_api_production_audit_v43511','authoritative_connectors_v43511','/public/authoritative-connectors/faostat/data','/public/authoritative-connectors/ilostat/indicator','/public/authoritative-connectors/oecd/sdmx','/public/authoritative-connectors/epa-frs/facilities','/public/authoritative-connectors/usgs-volcano/notices','/public/energy-systems/live/osm-power','/public/energy-systems/live/eia','/public/energy-systems/live/ember','/public/energy-systems/live/entsoe','/public/digital-connectivity/live/osm-telecom','/public/digital-connectivity/discovery/mlab','/public/digital-connectivity/discovery/fcc-bdc'):
...
print('PASS: v4.35.16 Authoritative Connector High-Priority Workspace Connector Closure I release contract')
