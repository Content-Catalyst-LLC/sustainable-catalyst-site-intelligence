...
c=TestClient(app)
s=c.get('/public/research-integration');assert s.status_code==200;s=s.json();assert s['version']=='4.14.0' and s['contract']=='research-evidence-and-knowledge-integration' and len(s['targets'])==4 and s['human_confirmation_required'] is True and s['automatic_delivery'] is False
...
html=(BACKEND/'public_app/index.html').read_text();worker=(BACKEND/'public_app/service-worker.js').read_text();js=(BACKEND/'public_app/assets/research-integration-v3270.js').read_text();assert 'research-integration-v3270.js?v=4.14.0' in html and 'research-integration-v3270.js' in worker and 'SCSIResearchIntegrationV3270' in js
policy=json.loads((BACKEND/'data/research_evidence_integration_policy_v3270.json').read_text());assert policy['version']=='4.14.0'
print(json.dumps({'version':s['version'],'targets':len(s['targets']),'manifest_fingerprint':manifest['manifest']['fingerprint'],'workbench_records':len(workbench['packet']['payload']['datasets'])},indent=2));print('PASS: Site Intelligence v4.14.0 research evidence and knowledge integration contracts are complete.')
