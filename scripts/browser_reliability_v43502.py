...
assert 'data-scsi-release="4.35.2"' in index
assert 'const VERSION="4.35.2"' in truth or "const VERSION='4.35.2'" in truth
assert "configurationRequired" in truth
assert "|unavailable|" not in truth
assert 'exoplanet-habitability-v43500.js?v=4.35.2' in astro
assert '"West Bank and Gaza": "Palestine"' in country

assert 'AUTHORITATIVE API COVERAGE' in index
assert 'authoritativeRegistrationMetric' in index
assert '/public/authoritative-apis' in app
assert 'renderAuthoritativeApiAudit' in app

print("PASS: v4.35.2 workspace reliability browser/static asset gate")
