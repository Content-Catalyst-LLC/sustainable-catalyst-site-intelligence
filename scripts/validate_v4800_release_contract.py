...
c=TestClient(app)
p=c.get('/public/underwater-observation').json(); assert p['ok'] and p['version']=='4.13.0' and p['contract']=='underwater-observation-visual-evidence' and p['route']=='earth' and p['source_count']==3 and p['media_type_count']==4
...
sea=c.get('/public/seafloor-intelligence/readiness').json(); assert sea['ok'] and sea['version']=='4.13.0'
nav=c.get('/public/v4/navigation').json(); assert nav['route_count']==35 and nav['primary_area_count']==6
print('PASS: v4.13.0 underwater observation / visual evidence release contract')
