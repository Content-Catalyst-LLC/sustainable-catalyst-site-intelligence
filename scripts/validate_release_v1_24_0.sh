...
VALIDATION_TMP="$(mktemp -d "${TMPDIR:-/tmp}/site-intelligence-v1240-validation.XXXXXX")"
...
for path in \
  ...
  backend/app/experience_quality.py \
  backend/tests/test_accessibility_performance_mobile_v1240.py \
  backend/public_app/index.html \
  backend/public_app/assets/app.js \
  backend/public_app/assets/app.css \
  wordpress-plugin/sustainable-catalyst-site-intelligence/sustainable-catalyst-site-intelligence.php \
  docs/ACCESSIBILITY_PERFORMANCE_MOBILE_V1240.md \
  docs/RELEASE_MANIFEST_V1240.json; do
  [[ -e "$ROOT_DIR/$path" ]] || fail "Missing required release path: $path"
done

say "Checking canonical release markers"
grep -q 'APP_VERSION = "1.24.0"' "$ROOT_DIR/backend/app/version.py" || fail "Backend version mismatch"
grep -q 'API_SCHEMA_VERSION = "1.24"' "$ROOT_DIR/backend/app/version.py" || fail "API schema version mismatch"
grep -q 'Version: 1.24.0' "$ROOT_DIR/wordpress-plugin/sustainable-catalyst-site-intelligence/sustainable-catalyst-site-intelligence.php" || fail "Plugin header mismatch"
grep -q "const VERSION = '1.24.0';" "$ROOT_DIR/wordpress-plugin/sustainable-catalyst-site-intelligence/sustainable-catalyst-site-intelligence.php" || fail "Plugin constant mismatch"
...
grep -q 'sc-experience-profile/1.0' "$ROOT_DIR/docs/RELEASE_MANIFEST_V1240.json" || fail "Experience profile release manifest schema missing"
grep -Eq '^##[[:space:]]+1\.24\.0([[:space:]]|$)' "$ROOT_DIR/CHANGELOG.md" || fail "CHANGELOG does not contain the 1.24.0 release heading"

grep -q '/public/experience-profile/diagnostics' "$ROOT_DIR/backend/app/main.py" || fail "Experience diagnostics endpoint missing"
grep -q 'id="mobileNavToggle"' "$ROOT_DIR/backend/public_app/index.html" || fail "Mobile navigation toggle missing"
grep -q 'ensureHtml2Canvas' "$ROOT_DIR/backend/public_app/assets/app.js" || fail "Lazy PNG loader missing"
grep -q 'forced-colors:active' "$ROOT_DIR/backend/public_app/assets/app.css" || fail "Forced-colors support missing"
grep -q 'prefers-reduced-motion:reduce' "$ROOT_DIR/backend/public_app/assets/app.css" || fail "Reduced-motion support missing"
...
say "Site Intelligence v1.24.0 validation passed"
