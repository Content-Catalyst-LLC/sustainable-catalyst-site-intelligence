...
if f'APP_VERSION = "{VERSION}"' not in version_py: fail("backend APP_VERSION mismatch")
if 'RELEASE_NAME = "Homepage Intelligence Contract & Ticker Recovery"' not in version_py: fail("release name mismatch")

summary_py=text("backend/app/homepage_summary_v4390.py")
for marker in ["enabled_connectors", "public_workspaces", "live_feeds", "featured_signal_count", "connector_operations_registry_v2130.json", "unified_public_intelligence_policy_v4000.json"]:
    if marker not in summary_py: fail(f"summary marker missing: {marker}")
for forbidden in ["registered_sources", "enabled_sources", "current_signals"]:
    if f'\"id\": \"{forbidden}\"' in summary_py: fail(f"superseded metric still active: {forbidden}")
...
for marker in ["Version: 4.40.0.2", "const VERSION = '4.40.0.2';", "site-intelligence-v4.40.0.2", "data-home-live-ticker", "['enabled_connectors', 'enabled connectors']", "['public_workspaces', 'public workspaces']", "['live_feeds', 'live ticker feeds']"]:
    if marker not in php: fail(f"WordPress marker missing: {marker}")
for marker in ["renderMetric('enabled_connectors', metrics.enabled_connectors)", "renderMetric('public_workspaces', metrics.public_workspaces)", "renderMetric('live_feeds', metrics.live_feeds)", "configureTickerTrack", "setupLiveIntelligence();"]:
    if marker not in js: fail(f"JavaScript marker missing: {marker}")
for forbidden in ["liveStatus.available_feeds.length", "liveStatus.default_feeds.length"]:
    if forbidden in js: fail(f"wrong metric fallback remains: {forbidden}")
for marker in ["scsi-home-summary__live-ticker", 'data-scsi-ticker-ready="1"', "translate3d(var(--scsi-live-travel),0,0)"]:
    if marker not in css: fail(f"ticker CSS marker missing: {marker}")
if "translateX(-50%)" in css: fail("legacy fixed -50% ticker travel remains")
...
if counts != expected: fail(f"current registries do not match homepage capability contract: {counts}")
...
for name in static_files:
    ...
    if payload.get("version") != VERSION: fail(f"{name}: release version mismatch")

print(json.dumps({"ok":True,"version":VERSION,"repair":"homepage-intelligence-contract-and-ticker","capability_metrics":counts,"featured_signal_count_separate":True,"integrated_homepage_ticker":True,"measured_ticker":True,"static_release_files":len(static_files)},indent=2))
print("PASS: Site Intelligence v4.40.0.2 release contract validated.")
