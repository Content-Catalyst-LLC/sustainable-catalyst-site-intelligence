...
def fixture_html():
    catalog={
      'ok':True,'version':'4.9.0','contract':'marine-biodiversity-bioacoustic-intelligence',
      'sources':[
...
window.fetch=async input=>{const u=String(input);let x=catalog;if(u.includes('/state')){const qp=new URL(u).searchParams,source=qp.get('source')||'obis',evidence=qp.get('evidence_class')||'occurrence-record',s=catalog.sources.find(r=>r.id===source)||catalog.sources[0],e=catalog.evidence_classes.find(r=>r.id===evidence)||catalog.evidence_classes[0];x={ok:true,version:'4.9.0',contract:'marine-biodiversity-bioacoustic-intelligence',source:s,evidence_class:e,scientific_name:qp.get('scientific_name')||null,point:qp.has('latitude')?{latitude:Number(qp.get('latitude')),longitude:Number(qp.get('longitude'))}:null,depth_m:qp.has('depth_m')?Number(qp.get('depth_m')):null,date:qp.get('date')||null,evidence:{records_loaded:false,record_count:null,presence_verified:false,absence_verified:false,abundance_verified:false,taxonomy_verified:false,acoustic_detection_verified:false},truth:{zero_results_as_absence:false,annotation_as_occurrence:false,model_detection_as_verified_species:false,recording_as_detection:false,detection_as_abundance:false,taxonomy_as_occurrence:false,environmental_context_assumed_synchronized:false}};}return new Response(JSON.stringify(x),{status:200,headers:{'Content-Type':'application/json'}})};
...
def exercise(page,label):
    ...
    page.wait_for_function("window.SCSIMarineBiodiversityV4900?.version==='4.9.0'")
    ...
    assert m['version']=='4.9.0' and m['source']=='onc-hydrophones' and m['evidence']=='acoustic-detection'
...
def main():
    ...
    print(json.dumps({'browser':path,'results':[r1,r2]},indent=2)); print('PASS: v4.9.0 Marine Biodiversity & Bioacoustic Intelligence passed direct and iframe interaction.'); sys.stdout.flush(); os._exit(0)
