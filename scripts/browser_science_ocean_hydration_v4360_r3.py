#!/usr/bin/env python3
"""Browser certification for v4.38.0 R3 Science-to-Ocean hydration and browser promotion.
...
def main() -> int:
    ...
    if not browser_path:
        print("ERROR: Chromium or Chrome is required for the v4.38.0 R3 Science browser gate.")
        return 2
    try:
        from playwright.sync_api import sync_playwright
    except ImportError:
        print("ERROR: Playwright is required for the v4.38.0 R3 Science browser gate.")
        return 2
    ...
    with sync_playwright() as pw:
        ...
        page.wait_for_function("document.querySelector('#oceanObservationStudio')?.dataset.oceanHydrationState === 'ready'", timeout=12000)
        page.wait_for_function("document.querySelectorAll('#oceanObservationStudio [data-ocean-card]').length === 11", timeout=12000)
        ocean = page.evaluate("""()=>({
          ...
          scienceStatus:document.querySelector('#scienceStatus span:last-child')?.textContent||''
        })""")
        assert ocean["visible"] is True and ocean["hydration"] == "ready" and ocean["cards"] == 11, ocean
    ...
    print("PASS: v4.38.0 R3 Science waits for Ocean hydration, certifies all 11 marine systems, and keeps Space discoverable with Platform Core unconfigured.")
    return 0
