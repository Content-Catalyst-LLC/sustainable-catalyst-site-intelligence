...
html=(BACKEND/'public_app/index.html').read_text();worker=(BACKEND/'public_app/service-worker.js').read_text();js=(BACKEND/'public_app/assets/monitoring-operations-v3280.js').read_text();assert 'monitoring-operations-v3280.js?v=4.1.0' in html and 'monitoring-operations-v3280.js' in worker and 'SCSIMonitoringOperationsV3280' in js
policy=json.loads((BACKEND/'data/monitoring_early_warning_policy_v3280.json').read_text());assert policy['version']=='4.1.0' and policy['prohibited']['automatic_publication'] is True
print(json.dumps({'version':s['version'],'alert_states':len(s['alert_states']),'watch_types':len(s['watch_types']),'sample_alert_state':evalp['alerts'][0]['state'],'digest_status':digest['digest']['status']},indent=2));print('PASS: Site Intelligence v4.1.0 Monitoring, Digests, and Early-Warning Operations contracts are complete.')
