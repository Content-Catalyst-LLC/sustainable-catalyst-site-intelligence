...
v4.16.0 turns the existing Alerts and Scheduled Monitoring capabilities into a coherent review-oriented monitoring layer. It adds explicit watchlists, geographic monitoring areas, source-change checks, threshold evaluation, alert-state transitions, modeled-warning separation, draft digest generation, and reviewed-feed contracts without claiming automatic emergency response or autonomous publication.
...
v4.16.0 does not provide automatic emergency dispatch, individual tracking, hidden risk scoring, automatic consequential action, fabricated events, or automatic publication.
