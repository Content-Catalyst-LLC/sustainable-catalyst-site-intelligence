...
"$PYTHON" "$ROOT/scripts/validate_v4900_release.py"
...
PY
if command -v node >/dev/null 2>&1; then node "$ROOT/scripts/check_javascript_v4900.js" "$BACKEND/public_app" "$ROOT/wordpress-plugin/sustainable-catalyst-site-intelligence/assets"; fi
...
"$PYTHON" "$ROOT/scripts/security_static_scan_v4900.py" "$ROOT"
if [[ "${SC_SI_SKIP_TESTS:-0}" != "1" ]]; then (cd "$BACKEND" && PYTHONPATH=. PYTEST_DISABLE_PLUGIN_AUTOLOAD=1 "$PYTHON" -m pytest -q); fi
if [[ "${SC_SI_RUN_BROWSER:-0}" == "1" ]]; then PYTHON="$PYTHON" "$PYTHON" "$ROOT/scripts/browser_marine_biodiversity_v4900.py"; fi
echo 'SUCCESS: Site Intelligence v4.14.0 passed Marine Biodiversity & Bioacoustic Intelligence validation.'
