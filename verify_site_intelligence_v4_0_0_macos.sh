...
printf '
==> Validating v4.15.0 Unified Public Intelligence Platform contracts
'
...
grep -q 'unified-platform-v4000.js?v=4.15.0' "$BACKEND/public_app/index.html"
grep -q 'unified-platform-v4000.css?v=4.15.0' "$BACKEND/public_app/index.html"
...
if [[ "${SC_SI_SKIP_TESTS:-0}" != "1" ]]; then
  printf '
==> Running complete inherited and v4.15.0 regression suite
'
  reset_runtime
...
if [[ "${SC_SI_RUN_BROWSER_GATE:-0}" == "1" && "${SC_SI_SKIP_BROWSER:-0}" != "1" ]]; then
  printf '
==> Running v4.15.0 browser consolidation gate
'
  reset_runtime
...
printf '
SUCCESS: Site Intelligence v4.15.0 passed Unified Public Intelligence Platform validation.
Repository: %s
' "$ROOT"
