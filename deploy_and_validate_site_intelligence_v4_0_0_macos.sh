...
'
SC_SI_STRICT_DEPENDENCIES=1 PYTHON="$ROOT/.venv/bin/python" bash "$ROOT/verify_site_intelligence_v4_0_0_macos.sh"
printf '
==> Running deterministic validation pass 2 (static/package assurance)
'
SC_SI_STRICT_DEPENDENCIES=1 SC_SI_SKIP_BROWSER=1 SC_SI_SKIP_TESTS=1 PYTHON="$ROOT/.venv/bin/python" bash "$ROOT/verify_site_intelligence_v4_0_0_macos.sh"
printf '
==> Promoting the backend through GitHub and Render
'
PYTHON="$ROOT/.venv/bin/python" bash "$ROOT/promote_site_intelligence_v4_0_0_to_github_and_render_macos.sh"
...
SUCCESS: Site Intelligence v${RELEASE} is live as the Unified Public Intelligence Platform.
...
Install the WordPress ZIP only now that the live gate has verified the exact release, Git commit, v4 platform contract, six-area navigation, all 35 preserved public routes, application shell, maps, analytical state, Data Truth, record provenance, assurance, research handoffs, monitoring, publication, governance, service-worker strategy, production truth, and runtime health.
DONE
