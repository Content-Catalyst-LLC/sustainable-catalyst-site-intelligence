...
The package is prepared for the existing resume-safe GitHub and Render promotion workflow. Production promotion is not performed by the build environment. Install the WordPress ZIP only after the installer verifies the exact v4.14.0 release id, Git commit, Render backend version, and live release gate.
