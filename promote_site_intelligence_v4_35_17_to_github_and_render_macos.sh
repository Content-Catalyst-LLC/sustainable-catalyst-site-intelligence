...
if [[ -x "$SOURCE_ROOT/.venv/bin/python" ]]; then SC_SI_STRICT_DEPENDENCIES=1 SC_SI_SKIP_BROWSER=1 PYTHON="$SOURCE_ROOT/.venv/bin/python" bash "$SOURCE_ROOT/verify_site_intelligence_v4_35_17_macos.sh"; else SC_SI_SKIP_BROWSER=1 bash "$SOURCE_ROOT/verify_site_intelligence_v4_35_17_macos.sh"; fi
...
if [[ -x "$SOURCE_ROOT/.venv/bin/python" ]]; then SC_SI_STRICT_DEPENDENCIES=1 SC_SI_SKIP_BROWSER=1 PYTHON="$SOURCE_ROOT/.venv/bin/python" bash "$CLONE_ROOT/verify_site_intelligence_v4_35_17_macos.sh"; else SC_SI_SKIP_BROWSER=1 bash "$CLONE_ROOT/verify_site_intelligence_v4_35_17_macos.sh"; fi
cd "$CLONE_ROOT"
...
if ! git diff --cached --quiet; then
  ...
  git commit -m "Site Intelligence v${RELEASE} — Rate Limits, Retries, Caching, Backoff & Circuit Breakers"
fi
...
for ((attempt=1; attempt<=ATTEMPTS; attempt++)); do
  ...
  if [[ -n "$gate" ]]; then
    ...
    if [[ "$OBSERVED_VERSION" == "$RELEASE" && "$OBSERVED_RELEASE_ID" == "$RELEASE_ID" && "$OBSERVED_GATE" == "ready" && ( "$OBSERVED_COMMIT" == "$COMMIT" || "$COMMIT" == "$OBSERVED_COMMIT"* || "$OBSERVED_COMMIT" == "$COMMIT"* ) ]]; then
      ...
      printf 'Release verification: health=%s runtime=%s deployment=%s routes=%s connectors=%s evidence=%s workspace-evidence=%s production-audit=%s source-policy=%s credential-control=%s resilience=%s app=%s\n' "$health_ready" "$runtime_ready" "$deployment_ready" "$v4_ready" "$connector_ready" "$evidence_ready" "$workspace_evidence_ready" "$production_audit_ready" "$source_health_ready" "$credential_ready" "$external_resilience_ready" "$app_ready"
      if [[ "$health_ready" == true && "$runtime_ready" == true && "$deployment_ready" == true && "$v4_ready" == true && "$connector_ready" == true && "$evidence_ready" == true && "$workspace_evidence_ready" == true && "$production_audit_ready" == true && "$source_health_ready" == true && "$credential_ready" == true && "$external_resilience_ready" == true && "$app_ready" == true ]]; then
