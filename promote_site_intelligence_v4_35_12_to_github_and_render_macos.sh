...
if [[ -x "$SOURCE_ROOT/.venv/bin/python" ]]; then SC_SI_STRICT_DEPENDENCIES=1 SC_SI_SKIP_BROWSER=1 PYTHON="$SOURCE_ROOT/.venv/bin/python" bash "$SOURCE_ROOT/verify_site_intelligence_v4_35_12_macos.sh"; else SC_SI_SKIP_BROWSER=1 bash "$SOURCE_ROOT/verify_site_intelligence_v4_35_12_macos.sh"; fi
...
if [[ -x "$SOURCE_ROOT/.venv/bin/python" ]]; then SC_SI_STRICT_DEPENDENCIES=1 SC_SI_SKIP_BROWSER=1 PYTHON="$SOURCE_ROOT/.venv/bin/python" bash "$CLONE_ROOT/verify_site_intelligence_v4_35_12_macos.sh"; else SC_SI_SKIP_BROWSER=1 bash "$CLONE_ROOT/verify_site_intelligence_v4_35_12_macos.sh"; fi
cd "$CLONE_ROOT"
...
if ! git diff --cached --quiet; then
  ...
  git commit -m "Site Intelligence v${RELEASE} — High-Priority Workspace Connector Closure II: Climate & Atmosphere"
fi
  ...
  if [[ -n "$gate" ]]; then
    ...
    if [[ "$OBSERVED_VERSION" == "$RELEASE" && "$OBSERVED_RELEASE_ID" == "$RELEASE_ID" && "$OBSERVED_GATE" == "ready" && ( "$OBSERVED_COMMIT" == "$COMMIT" || "$COMMIT" == "$OBSERVED_COMMIT"* || "$OBSERVED_COMMIT" == "$COMMIT"* ) ]]; then
      ...
      if [[ "$health_ready" == true && "$runtime_ready" == true && "$deployment_ready" == true && "$v4_ready" == true && "$connector_ready" == true && "$evidence_ready" == true && "$workspace_evidence_ready" == true && "$production_audit_ready" == true && "$source_health_ready" == true && "$app_ready" == true ]]; then
        ...
        printf '\nSUCCESS: Site Intelligence v%s is live with High-Priority Workspace Connector Closure II: Climate & Atmosphere at commit %s.\nDeployment receipt: %s\nRollback tag: %s (%s)\n' "$RELEASE" "${OBSERVED_COMMIT:0:12}" "$DEPLOYMENT_RECEIPT" "$ROLLBACK_TAG" "${PREVIOUS_COMMIT:0:12}"
        exit 0
      fi
    fi
  fi
