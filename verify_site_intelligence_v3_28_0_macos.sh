...
RUNTIME_SANDBOX="$(mktemp -d "${TMPDIR:-/tmp}/scsi-v3280-runtime.XXXXXX")"
...
printf '\n==> Validating v4.14.0 Monitoring, Digests, and Early-Warning Operations contracts\n'
PYTHONPATH="$BACKEND" "$PYTHON" "$ROOT/scripts/validate_v3280_release.py"
...
grep -q 'monitoring-operations-v3280.js?v=4.14.0' "$BACKEND/public_app/index.html"
grep -q 'monitoring-operations-v3280.js' "$BACKEND/public_app/service-worker.js"
grep -q 'SCSIMonitoringOperationsV3280' "$BACKEND/public_app/assets/monitoring-operations-v3280.js"
cmp -s "$BACKEND/public_app/assets/monitoring-operations-v3280.js" "$ROOT/wordpress-plugin/sustainable-catalyst-site-intelligence/assets/monitoring-operations-v3280.js"
cmp -s "$BACKEND/public_app/assets/monitoring-operations-v3280.css" "$ROOT/wordpress-plugin/sustainable-catalyst-site-intelligence/assets/monitoring-operations-v3280.css"
...
if command -v node >/dev/null 2>&1; then
  ...
  node "$ROOT/scripts/check_javascript_v3280.js" "$BACKEND/public_app" "$ROOT/wordpress-plugin/sustainable-catalyst-site-intelligence/assets"
fi
...
printf '\n==> Running complete inherited and v4.14.0 test suite\n'
reset_runtime
PYTHON="$PYTHON" PYTHONPATH="$BACKEND" PYTEST_DISABLE_PLUGIN_AUTOLOAD=1 "$PYTHON" "$ROOT/scripts/run_v3280_test_suite.py"
rm -rf "$BACKEND/backend"
[[ ! -e "$BACKEND/backend" ]] || { echo 'ERROR: runtime state cleanup failed.' >&2; exit 1; }
printf '\nSUCCESS: Site Intelligence v4.14.0 passed Monitoring, Digests, and Early-Warning Operations validation.\nRepository: %s\n' "$ROOT"
